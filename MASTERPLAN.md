...
- **E4 — Money-Modell-Glattzug + Mock→Pro E2E-Härtung (D99) — NEU, 2026-06-24:** Ausgelöst durch den Slice-365-E2E-Durchlauf: **ein einziger echter Trade** deckte systemischen Drift + mehrere Bugs auf → Beleg, dass „Mock→Pro" eine **gezielte Härtungs-Welle** braucht, nicht nur Feature-Bau. Zwei Arbeitsströme:
  - **(a) Money-Modell-Doku auf D99 ausrichten** (Anti-Drift, „Wissen verbunden"): ~40 Stellen, Checkliste `worklog/notes/365-money-model-drift-inventory.md`, SSOT = **D99** (`memory/decisions.md`). **ZUERST Anil-Ratifikation** der D99-OFFEN-Punkte (Naming Credits-vs-$SCOUT · Einheiten-Vokabular · Phasen-Nummerierung 1/3/4↔1/2 · CASP-Strategie · Pricing-Fairness), DANN Massen-Glattzug. **Kein Edit vor Ratifikation.**
  - **(b) E2E-Sweep „Fees REIN" zu Ende + Bugs fixen:** ① Trading ✅ (Topf +35 live bewiesen) → ② IPO → ③ Poll → ④ Research → ⑤ Bounty (je Seed + echter UI-Trigger, Bug-Jagd). Funde so far in `worklog/notes/365-e2e-findings.md`: **T-3 „Diamond Hands"** (verbotenes Meme-Coin-Wort + Award-on-buy-Logikbug + Konfetti-auf-Trade, HIGH/Compliance) · **T-2** `/api/push 500` · **T-1** Cold-Start leerer Markt (keine Kauf-Liquidität) · **P-4** `ipo_price`-MV-Drift (Nicht-Top-Spieler verzerrt) · **M-5** = der Money-Modell-Drift (→ D99).
...
## 📍 Stand in einem Satz
Treasury-RAUS-Kanäle ✅ + Operating-System (E0) ✅ + **Polls-Geldmaschine komplett (333-337+343) ✅ live** + **Fan-Reward-Engine FRE-1/2/3/5 ✅ live** (344-347) + **Polls-Roadmap komplett (356)**. **E3 Plattform-Treasury (D96) IM BAU: Slice 1 Topf-Fundament ✅ (357) + Slice 2 „Fees REIN" KOMPLETT ✅ (358 Trading + 360 IPO + 363 Polls + 364 Research + 365 Bounty, + P2P)** — ALLE Plattform-Fee-Ströme fließen real in den Topf (voller Auffang 100 %, D98, je Zero-Sum live bewiesen). **Nächster = Slice 3 Monats-Liga e2e (erster RAUS-Kanal aus dem Topf).** Sequenz 1✅→2 Fees REIN ✅ (5/5)→3 Liga e2e→4 Events→5 Wettkampf-Darstellung. Plan `worklog/notes/358-platform-treasury-epic.md`.

**⚠️ Dazwischengeschoben (2026-06-24, Anil-Prio): E4 Money-Modell-Glattzug + Mock→Pro-E2E-Härtung (D99).** Der Slice-365-E2E-Durchlauf bewies an EINEM echten Trade, wie viel Drift/Bugs in einer simplen Ausführung stecken → erst sauber glattziehen (Money-Modell-Doku auf D99, dann E2E-Sweep ②–⑤ + Bug-Fixes), Reihenfolge mit Anil. Trackers: `365-money-model-drift-inventory.md` + `365-e2e-findings.md`. **Anil knüpft hier an.**
