...
- **W0 — DB-Security [§3, Triage fertig]:** ✅ **INV-31 REVOKE (460, live):** `calculate_fan_rank` + `refund_wildcards_on_leave` (Info-Leak + Self-Farm zu). · ✅ **D-12 DROP (461, live):** toter `get_club_dashboard_stats(text)` v1 weg. · ✅ **D-35 (462, live):** `get_club_dashboard_stats_v2` Admin-Guard (club_admin OR platform_admin) + REVOKE anon — Club-Finanz/Fan-PII-Exposure zu (Reviewer PASS, kanonische `platform_admins`-Quelle). · **OFFEN:** **D-36** 2 Stats-Siblings (`rpc_get_club_trading_fees`/`fan_stats`) dead `top_role`-Branch → auf `platform_admins` (Reviewer „priorisieren") · 2 Recon-RPCs (`security_definer_audit`, `rls_policy_matrix`) admin-only · `anon` REVOKE-Hygiene (9 Trigger + ~10 Kalkulatoren + 3 Leaderboard-RPCs) · 81 permissive Policies + 26 unused Index + 51 FK-Index. *(Verifiziert: kein aktiver Leak.)*
