...
- **K2 — EINE Wissens-Heimat (Epic, 6 Wellen — Recon 2026-06-29):** Ist-Stand = 3 Skill-Trees + 3 Wissens-Heimaten + Dubletten. Ziel `docs/knowledge/` = SSOT.
  ...
  - **K2.2** memory-Müll-GC: ✅ **Slice 442 (Teil):** leeres `semantisch/` (D89-tot) weg. ⬜ **K2.2b:** `debug-backfill-payload` (220K) + `rollback_*.json` (an 4 one-off Scripts) + `beta-*`-Docs (12, referenziert in auditor/errors-infra/beta-metrics) = scripts-GC-/Urteils-Thema (435-Nachzug), kein Blind-Schnitt.
  - **K2.3** docs-root: 4 versionierte Dubletten (`Context_Pack_v8`/`SYSTEM-DESIGN-v2`/`final-report-v3`/`gamification-v4-FINAL`) + `test.rtf` → je 1 aktuelle / `docs/knowledge`.
