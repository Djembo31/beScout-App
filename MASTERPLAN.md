...
- **K2 — EINE Wissens-Heimat:** `docs/knowledge/` = SSOT. `wiki/` (21) + `memory/semantisch/` (3, D89-tot) + wertvolle `docs/`-root-Docs reinmigrieren; **versionierte Dubletten** (`…Context_Pack_v8`, `SYSTEM-DESIGN-v2`, `…report-v3`, `…gamification-v4-FINAL`) auf je 1 aktuelle ziehen, Rest löschen. `disease-register` → hierher (= verifizierte Evidenz). **+ NEU aus K1 (439):** die ~13 lebenden `worklog/notes/`-Plan-Doc-Anker (epics/recon/audits) hierher migrieren UND die Inbound-Refs in `decisions.md`/`INDEX.md`/`treasury.md`/`session-handoff.md` auf den neuen Pfad umbiegen (sonst dangling). Memory-Modell klären (Projekt-Vault vs Auto-Memory: 1 Modell).
