...
**Querschnitt-Konsolidierungen (in die Wellen verteilt):** Score-SSOT (`scout_scores`+`user_stats`+`bescout_scores`(tot) → 1 Quelle, W2) — ✅ **D-17 geheilt (Slice 454):** user_stats-Scores = kept-fresh Projektion von scout_scores (Divergenz 70→0 live, Projektions-Trigger, Money-Anker 0 Edits, Reviewer-Level-Kaskade geguarded). Residual: Path 2 (Score-Spalten droppen) + D-11 (totes bescout_scores) · 3 Kauf-RPCs angleichen + `credit_pbt` kanonisieren (W1/W5) · 2 Mission-Systeme (`scout_missions` tot → löschen, W4) · `players.club` String→UUID (⛔ blockiert auf API-Football-Key, W2/W3).
