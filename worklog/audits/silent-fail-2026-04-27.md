...
## Pattern: in-without-chunking (119 findings)
...
- `src/app/api/cron/gameweek-sync/route.ts:243` [HIGH] — .in('id', leagueIds)
- `src/app/api/cron/gameweek-sync/route.ts:493` [HIGH] — .in('club_id', clubsToProcess.map(c => c.id))
- `src/app/api/cron/gameweek-sync/route.ts:523` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:645` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:653` [HIGH] — .in('club_id', allLeagueClubIds),
- `src/app/api/cron/gameweek-sync/route.ts:752` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:1360` [HIGH] — .in('home_club_id', allLeagueClubIds);
- `src/app/api/cron/gameweek-sync/route.ts:1444` [HIGH] — .in('event_id', eventIds);
- `src/app/api/cron/gameweek-sync/route.ts:1521` [HIGH] — .in('home_club_id', allLeagueClubIds);
- `src/app/api/cron/gameweek-sync/route.ts:1646` [HIGH] — .in('club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:1659` [HIGH] — .in('club_id', allLeagueClubIds)
- `src/components/profile/FollowListModal.tsx:59` [MEDIUM] — .in('user_id', userIds);
- `src/features/fantasy/services/events.mutations.ts:309` [MEDIUM] — .in('id', eventIds);
- `src/features/fantasy/services/events.queries.ts:39` [MEDIUM] — .in('club_id', clubIds)
- ... +69 more

## Pattern: error-check-without-throw-or-return (50 findings)
...
- `src/lib/services/club.ts:696` [MEDIUM] — if (error) return { success: false, error: error.message };
...
- `src/lib/services/__tests__/club-most-owned-batch.test.ts:64` [MEDIUM] — if (error) {
- `src/lib/services/__tests__/club-most-owned-batch.test.ts:286` [MEDIUM] — if (error) return;
- `src/lib/services/__tests__/club-most-owned.test.ts:63` [MEDIUM] — if (error) {
- `src/lib/services/__tests__/club-most-owned.test.ts:198` [MEDIUM] — if (error) return;
- `src/lib/services/__tests__/events-difficulty.test.ts:193` [MEDIUM] — if (error) return;
- `src/lib/services/__tests__/leaderboards.test.ts:206` [MEDIUM] — if (error) {
- `src/lib/services/__tests__/lineup-auto-sub.test.ts:152` [MEDIUM] — if (error) {
- `src/lib/services/__tests__/lineup-auto-sub.test.ts:219` [MEDIUM] — if (error) {
