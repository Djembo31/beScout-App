...
## Pattern: in-without-chunking (104 findings)
...
- `scripts/backfill-complete-stats.mjs:553` [MEDIUM] — .in('fixture_id', gwFixtureIds);
...
- `scripts/cron-health-check.ts:151` [MEDIUM] — .in('home_club_id', clubIds)
- `scripts/cron-health-check.ts:167` [MEDIUM] — .in('home_club_id', clubIds)
...
- `scripts/tm-rescrape-stale.ts:327` [MEDIUM] — .in('club_id', clubIds);
- `scripts/tm-rescrape-stale.ts:332` [MEDIUM] — .in('club_id', clubIds);
...
- `src/app/api/cron/gameweek-sync/route.ts:246` [HIGH] — .in('id', leagueIds)
- `src/app/api/cron/gameweek-sync/route.ts:639` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:775` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:783` [HIGH] — .in('club_id', allLeagueClubIds),
- `src/app/api/cron/gameweek-sync/route.ts:882` [HIGH] — .in('home_club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:1490` [HIGH] — .in('home_club_id', allLeagueClubIds);
- `src/app/api/cron/gameweek-sync/route.ts:1574` [HIGH] — .in('event_id', eventIds);
- `src/app/api/cron/gameweek-sync/route.ts:1652` [HIGH] — .in('home_club_id', allLeagueClubIds);
- `src/app/api/cron/gameweek-sync/route.ts:1785` [HIGH] — .in('club_id', allLeagueClubIds)
- `src/app/api/cron/gameweek-sync/route.ts:1798` [HIGH] — .in('club_id', allLeagueClubIds)
- `src/app/api/cron/live-score-sync/route.ts:172` [HIGH] — .in('id', leagueIds)
- `src/components/profile/FollowListModal.tsx:59` [MEDIUM] — .in('user_id', userIds);
- `src/features/fantasy/services/events.mutations.ts:314` [MEDIUM] — .in('id', eventIds);
- `src/features/fantasy/services/events.queries.ts:39` [MEDIUM] — .in('club_id', clubIds)
- `src/features/fantasy/services/events.queries.ts:130` [MEDIUM] — query = query.in('status', filters.status);
- ... +54 more

## Pattern: error-check-without-throw-or-return (42 findings)
...
- `src/lib/services/club.ts:710` [MEDIUM] — if (error) return { success: false, error: error.message };
...
- `src/lib/services/liquidation.ts:49` [MEDIUM] — if (error) return { success: false, error: mapRpcError(error.message) };
- `src/lib/services/missions.ts:134` [MEDIUM] — if (error) return { success: false, error: mapErrorToKey(normalizeError(error)) };
- `src/lib/services/mysteryBox.ts:48` [MEDIUM] — if (error) {
- `src/lib/services/players.ts:264` [MEDIUM] — if (error) return { success: false, error: error.message };
...
- `src/lib/services/footballData.ts:397` [HIGH] — supabase.from('players').select('id', { count: 'exact', head: true }),
...
- `src/lib/__tests__/db-invariants.test.ts:83` [HIGH] — const { data: allClubs } = await sb.from('clubs').select('id, league_id');
- `src/lib/__tests__/db-invariants.test.ts:146` [HIGH] — const { data: allClubs } = await sb.from('clubs').select('id, league_id');
- `src/lib/__tests__/db-invariants.test.ts:1904` [HIGH] — const { data, error } = await sb.from('clubs').select('id, name, logo_url');
...
- `src/app/api/admin/backfill-ratings/route.ts:208` [HIGH] — const { data } = await supabaseAdmin.rpc('admin_resync_gw_scores', { p_gameweek: gw });
...
- `src/lib/supabaseMiddleware.ts:77` [HIGH] — const { data } = await supabase.auth.getUser();
