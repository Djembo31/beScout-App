...
- **Fix (live, Migration `20260630130000`):** beide top_role-Check auf kanonische `platform_admins` (get_sponsor qualifiziert `public.` wg. `SET search_path TO ''`) + **REVOKE anon** get_sponsor. Bodies byte-true (RETURN QUERY / UPSERT + recompute-Loop). set_thresholds club_admins-Branch unverändert. `calculate_fan_rank`-PERFORM läuft im SECDEF-Owner-Kontext → von 460-REVOKE unberührt.
- **Proof:** post-apply beide `platform_admins=true`/`top_role=false`, get_sponsor anon REVOKEd, club_admins-Branch intakt + **`family_remaining=0`** (top_role-Familie restlos, 7 RPCs über D-36/37/37b) + 3-Rollen-Smoke (non-admin reject / platform-admin past / club-admin past) + **Recompute-Happy-Path** (Club mit 37 fan_rankings → success=true recalculated=37, SECDEF-Owner-Recompute bewiesen, rolled back) + tsc 0 + db-invariants unverändert 3 + fanRanking-Tests grün. Proof `465-d37b-family-close.txt`, Review `465-review.md` (PASS).
- **Knowledge:** errors-db **S465** (Proof-Rigor: Guard-only-Smoke mit invalid Input lässt Write-/Recompute-Pfad unexerziert → Happy-Path-force-rollback mit assert side-effect-count>0 Pflicht; `EXCEPTION…CONTINUE`-Loop schluckt Permission-Fehler still). Disease-Register: **D-37b → geheilt** (Familie KOMPLETT), **D-38** neu (sponsorStats.ts Silent-Fail, Reviewer-Nit). MASTERPLAN W0.
- **Bonus:** get_sponsor anon-REVOKE behebt ein echtes latentes Security-Loch (vestigial anon auf admin-read), nicht nur Konsistenz.

## 464 | 2026-06-30 | fix(security): D-37 — SOLE-gate top_role-RPCs auf platform_admins (grant_founding_pass + admin_grant_wildcards + cancel_event_entries) (live)
- Stage-Chain: SPEC → IMPACT (skipped) → BUILD (1 Migration, 3 RPCs) → PROVE (3-Rollen-force-rollback + voller Money-Mint-rollback) → REVIEW (Cold-Context **PASS**, §3 Money streng) → CEO-Apply → LOG. **Security/Money §3 — selbst; CEO autonom-Go Anil „mach autonom weiter".**
- **Befund (verifiziert read-only):** 3 live-verdrahtete Admin-RPCs (`grant_founding_pass` foundingPasses.ts:40 = **MONEY**/bcredits-Mint+Kill-Switch, `admin_grant_wildcards` wildcards.ts:72 = **MINTING**, `cancel_event_entries` events.mutations.ts:624) gateten auf `profiles.top_role='Admin'` = **0 Profile global** (SOLE-gate, kein club_admins-Fallback, kein service_role-Bypass) → **always-reject = effektiv TOT**. Fail-closed (kein Leak), aber 3 kaputte Admin-Capabilities (2 Money/Minting).
- **Fix (live, Migration `20260630120000`):** Guard je RPC auf kanonische `platform_admins` (= D-36/v2/get_club_balance/UI `isPlatformAdmin`). Money/Minting-Bodies (CASE-Tiers 250000/1M/3.5M/10M, Kill-Switch 90000000, wallet/transactions/wildcard-INSERT) byte-treu (PATCH-AUDIT, Reviewer byte-diff gg. Vorgänger-Migrationen verifiziert). admin_grant_wildcards: auth_uid_mismatch-Spoof-Guard erhalten. Unbenutzte DECLARE-Vars raus. AR-44-Block.
- **Proof:** post-apply alle 3 `uses_platform_admins=true`/`still_top_role=false`/Spoof-Guard erhalten + 3-Rollen-Smoke (JWT-sub S369: non-admin reject ×3 / platform-admin past-guard ×3 / **voller Founding-Pass-Mint ok=true bcredits=250000 in Tx ROLLED BACK**) + INV-31 grün (kein Guard verloren) + tier-invariant grün + tsc 0 + db-invariants unverändert 3. Proof `464-d37-soletop-role.txt`, Review `464-review.md` (PASS).
- **Permissive-only:** da die RPCs vorher JEDEN ablehnten, kann der Fix nur restaurieren (Platform-Admin-Zugang), nichts verschlechtern. Kill-Switch unangetastet, kein neuer Money-Flow, Nicht-Platform-Admin kann nirgends minten.
- **Knowledge:** errors-db **S463-Erw.** (SOLE-gate-Swap = permissive-only-Move; Vollständigkeits-Audit-Pflicht NACH Fix). Disease-Register: **D-37 → geheilt**, **D-37b** neu (post-464-Audit fand 2 weitere top_role-RPCs → 465). MASTERPLAN W0 reconciled.
- **🟡 Residual → 465 (Reviewer + Post-Audit):** `get_sponsor_stats_summary` (SOLE-gate read-only) + `set_club_fan_rank_thresholds` (Sekundär-Branch) = Rest der `top_role='Admin'`-Familie. Plus Reviewer-INFO: grant_founding_pass unqualifiziertes `platform_admins` (kosmetisch, search_path-Härtung = eigener W0-Sweep) + Kill-Switch-SUM-ohne-Lock (pre-existing, byte-treu, CEO-Scope).

## 463 | 2026-06-29 | fix(security): D-36 — Stats-Siblings Platform-Admin-Guard auf platform_admins (live)
- Stage-Chain: SPEC → IMPACT (skipped: 0 TS-Consumer-Change) → BUILD (1 Migration, 2 RPCs) → PROVE (post-apply 3-Rollen-Smoke + Body-Anker) → REVIEW (Cold-Context **CONCERNS** = Scope-Out, Slice selbst mergeable) → CEO-Apply → LOG. **Security/Konsistenz §3; CEO Anil „mach d36". Fortsetzung S462.**
- **Befund (D87 Live):** `rpc_get_club_trading_fees` + `rpc_get_club_fan_stats` (SECDEF, admin-tab-only) prüften Platform-Admin per `profiles.top_role='Admin'` = **0 Match (dead branch)**; club_admins-Branch funktionierte (Sekundär). Seit S462 sichtbar inkonsistent: AdminRevenueTab — v2 (Platform-Admin via `platform_admins`) liefert Daten, Sibling RAISEt.
- **Fix (live, Migration `20260629240000`):** beide RPCs je **1 Guard-Zeile** auf kanonische Quelle `platform_admins` (= v2/get_club_balance/UI-Quelle `isPlatformAdmin`). Body byte-treu (PATCH-AUDIT), club_admins-Branch unverändert → rein permissiver Fix (repariert toten Override). Kein Grant-Change (anon schon revoked).
- **Proof:** post-apply `uses_platform_admins=true`/`still_top_role=false`/`club_admin_branch_intact=true` (beide) + 3-Rollen-Smoke (JWT-sub S369: nonadmin rej/rej · club-admin ok/ok · **platform-admin ohne club-Row ok/ok = reparierter Branch**) + Body-Anker intakt + `remaining_toprole_in_family=0` + tsc 0 + club.test 79/79. Proof `463-d36-sibling-guard.txt`, Review `463-review.md`.
- **🔴 REVIEWER-CATCH (MEDIUM, Scope-Out → D-37, PRIORISIERT):** 3 weitere RPCs nutzen `top_role='Admin'` als **ALLEINIGES** Gate (kein club_admins-Fallback) → bei 0-Match komplett TOT/unzugänglich: **`grant_founding_pass`** (Money/Kill-Switch), **`admin_grant_wildcards`** (Minting), **`cancel_event_entries`**. Anders als D-36 = Total-Lockout-Risiko, nicht nur Override-Verlust. NICHT Scope 463 → D-37 (erst verifizieren ob live tot). `set_club_fan_rank_thresholds` = Sekundär (wie D-36); `prevent_profile_sensitive_update` = False-Positive (friert Spalte).
- **Knowledge:** errors-db **S463** (dead-top_role-Restfamilie nach Gate-Topologie klassifizieren: SOLE-gate=Total-Lockout vs Sekundär=Override-Verlust; False-Positive = einfrierender Trigger). Disease-Register: **D-36 → geheilt**, **D-37** neu (🔴 priorisiert, Money/Minting). MASTERPLAN W0 reconciled.
- **Scope-Out:** D-37 (eigene §3-Investigation) · weitere W0-Reste.

## 462 | 2026-06-29 | fix(security): D-35 — get_club_dashboard_stats_v2 Admin-Guard + REVOKE anon (live)
- Stage-Chain: SPEC → IMPACT (skipped: 0 TS-Consumer-Change, RPC-intern) → BUILD (1 Migration) → PROVE (pre-apply force-rollback 3 Rollen + post-apply Live-Recheck) → REVIEW (Cold-Context **PASS**) → CEO-Apply → LOG. **Security §3; CEO Anil „Komplett: REVOKE anon + Admin-Guard".**
- **Befund (D87 Live, Carry-forward D-35 aus S461):** `get_club_dashboard_stats_v2(uuid)` (SECDEF) ohne Guard + anon-granted → jeder anon/authenticated konnte für JEDEN Club `ipo_revenue_cents` (Club-Finanz) + Top-Fan-PII (`user_id`/`holdings_count`/`total_score`) lesen. Konsumenten = nur AdminOverviewTab/AdminRevenueTab (admin-gated). S095-Verletzung.
- **Recon-Fund (verhinderte Regression):** zwei konkurrierende Platform-Admin-Muster — kanonisch (22 RPCs, `get_club_balance`): `platform_admins`-Tabelle (real); 2 Stats-Siblings: `profiles.top_role='Admin'` = **0 Match (dead branch)**. v2 spiegelt die kanonische Variante; blinde Sibling-Kopie hätte echte Platform-Admins für fremde Clubs ausgesperrt.
- **Fix (live, Migration `20260629230000`):** Guard byte-exakt aus `get_club_balance` am BEGIN (`v_caller IS NULL→RAISE auth_required` + `club_admins(p_club_id)` OR `platform_admins`, sonst RAISE not_authorized) + `REVOKE EXECUTE FROM anon, PUBLIC` + `GRANT authenticated`. v2-Body sonst byte-treu (PATCH-AUDIT, nur Guard additiv).
- **Proof:** pre-apply force-rollback (neue Version in Tx, 3 Rollen via JWT-sub-Impersonation: nonadmin rejected / club-admin ok / platform-admin ok via `platform_admins`, pa_is_clubadmin=false) + post-apply (anon_exec=FALSE, auth/service=TRUE, Guard+Body-Anker intakt) + Live-Re-Confirm + tsc 0 + club.test 79/79 + db-invariants unverändert 3 (INV-19/32/33). Proof `462-d35-admin-guard.txt`, Review `462-review.md` (PASS).
- **Knowledge:** errors-db **S462** (S347-Erweiterung: `platform_admins` kanonisch vs dead `top_role`; neuen Guard gegen UI-Quelle diffen = Reverse-S347-Falle; SECDEF-Proof = voller functiondef-Diff). Disease-Register: **D-35 → geheilt**, **D-36** neu (2 Stats-Siblings dead top_role-Branch → auf platform_admins, Reviewer „priorisieren"). MASTERPLAN W0 reconciled.
- **Scope-Out:** D-36 Sibling-Guard-Konsistenz (eigene RPCs, eigener Proof) · weitere W0-Reste (Recon-RPCs admin-only, anon-Hygiene-Batch, Policy/Index).

## 461 | 2026-06-29 | chore(db): D-12 Dead-RPC GC — DROP get_club_dashboard_stats(text) v1 (live)
- Stage-Chain: SPEC → IMPACT (skipped: 0 Consumer, reiner DROP) → BUILD (1 Migration) → PROVE (pre-drop force-rollback + 5 ACs) → REVIEW (Cold-Context **PASS**) → CEO-Apply → LOG. **Security/Dead-GC §3, §0-Subtraktion; CEO Anil „mach D-12".**
- **Befund (D87 Live):** `get_club_dashboard_stats(p_club_name text)` = v1 (by name) = **tot** (3-Wege-Enum: pg_proc.prosrc leer + 0 pg_cron + Grep src/+supabase/+scripts/ nur `_v2`; Vorab-Audit `2026-06-14/string-to-uuid-map.md:45` bestätigt). SECDEF + **anon-granted**, liest RLS-umgehend `holdings`/`user_stats`, gibt pro Fan `user_id`/`holdings_count`/`total_score` zurück, per `club_name` enumerierbar. Body nutzt toten Freitext-`players.club`-Pfad (D-26). `pg_depend`=0 → DROP ohne CASCADE. Live-Pfad = `get_club_dashboard_stats_v2(uuid)` (`club.ts:503`).
- **Fix (live, Migration `20260629220000`):** `DROP FUNCTION IF EXISTS public.get_club_dashboard_stats(text)`. Reine Subtraktion (§0).
- **Proof:** pre-drop force-rollback (v1 1→0, v2-Survivor=1, rolled back); post-apply: nur noch `_v2(uuid)` (v1 weg), v2 auth/anon_exec=TRUE, v2-Call ok (total_fans=2 force-rollback), db-invariants **unverändert 3 failed** (INV-19/32/33 pre-existing, keine neue, kein get_club_dashboard-Bezug), tsc 0, club.test.ts 79/79. Proof `461-d12-drop.txt`, Review `461-review.md` (PASS).
- **⚠️ EHRLICHER SCOPE (Reviewer-Catch #1):** DROP entfernt **toten v1-Pfad + by-name-Enumerations-Vektor + anon-SECDEF-Surface −1** — schließt **NICHT** die Kern-PII-Exposure: `get_club_dashboard_stats_v2(uuid)` ist ebenfalls anon-granted + liefert identische Shape inkl. `user_id`/`holdings_count` (RPC-Shape-Audit `007:132-133`) → **D-35** (v2-anon-Grant-Entscheid, W0-Hygiene). Kein „anon-PII-Leak geschlossen".
- **Knowledge:** errors-db **S461** (Dead-RPC-v1-DROP: v2-Shape/Grants gegen v1 diffen, sonst überzeichnet die Closure; plain DROP ohne CASCADE = Dependent-Beweis). Disease-Register: **D-12 → geheilt**, **D-35** neu (v2-anon-Exposure). MASTERPLAN W0 reconciled.
- **Scope-Out:** D-35 v2-anon-Grant · D-26 Freitext-club-Pfad · W0-Rest (audit-RPCs admin-only + Hygiene-REVOKE-Batch + Policy/Index-Konsolidierung).

## 460 | 2026-06-29 | fix(security): INV-31 — REVOKE no_guard SECDEF-RPCs (calculate_fan_rank + refund_wildcards_on_leave) (live)
- Stage-Chain: SPEC → IMPACT (skipped: reines Grant-REVOKE, 0 Code-Consumer) → BUILD (1 Migration) → PROVE (5 ACs live + force-rollback) → REVIEW (Cold-Context **PASS**) → CEO-Apply → LOG. **Security §3; CEO Anil „INV-31 jetzt, REVOKE-only".**
- **Befund (D87 Live, INV-31 rot):** 2 SECURITY-DEFINER-RPCs mit `p_user_id`-Param, an `authenticated` granted, **ohne `auth.uid()`-Guard** (`no_guard`) → ein eingeloggter User kann fremde `p_user_id` übergeben (identity-spoof). **`refund_wildcards_on_leave`** = toter Orphan (0 Caller src/+supabase/+scripts/+pg_proc), ruft `earn_wildcards` (Wert-Item, keine Idempotenz) → authenticated Self-Repeat-Wildcard-Farm via PostgREST (cross-user durch inneren earn_wildcards-Guard blockiert). **`calculate_fan_rank`** = live nur via service_role (Cron) + SECDEF-Owner (Trigger/Batch); Client-Service `recalculateFanRank` tot → Direkt-Spoof = Info-Leak (Holdings-Count/Abo-Tier in Rückgabe).
- **Root-Cause (Reviewer-Catch):** Slice 251 (`20260428120500` Per-Liga-Wildcards-Rewrite) ließ den AR-27-Guard (`20260414200000`) für genau `refund_wildcards_on_leave` still fallen — S156-Silent-Revert, 4/5 Geschwister behielten ihn, ~2 Monate latent. INV-31 als **laufender** Invariant fing es (nicht Code-Review).
- **Fix (live, Migration `20260629210000`):** REVOKE-only — `REVOKE EXECUTE … FROM authenticated, anon, PUBLIC` auf beide. Kein Body-Rewrite (null S156-PATCH-AUDIT-Risiko am 5k-Zeichen-Body). Beide haben keinen legitimen direkten authenticated-Caller (Cron/Batch/Trigger = service_role/Owner-Kontext) → Lecks zu, alle echten Caller laufen.
- **Proof:** vorher/nachher-ACL `{postgres,service_role}` + `has_function_privilege` (auth+anon→FALSE, service_role→TRUE beide) + `get_security_definer_user_param_audit` needs_fix=0 + force-rollback Owner-Call (`calculate_fan_rank` ok=true tier=stammgast, UPSERT rolled back) + volle Suite **4→3 failed** (INV-31 grün, INV-19/32/33 unverändert pre-existing, keine neue) + tsc 0. Proof `460-inv31-revoke.txt`, Review `460-review.md` (PASS, „ein Senior merged das").
- **Knowledge:** errors-db **S460** (Batch-Rewrite verliert Guard für 1/N RPC still → laufender Invariant statt Einmal-Audit; REVOKE-only vs Guard-Wahl; REVOKE schließt Aufruf-Fläche nicht Body-Schwäche). Disease-Register: INV-31 → **geheilt**; **D-34** neu (toter non-idempotenter Orphan `refund_wildcards_on_leave` grant-dicht, Re-Arm-Vektor bei service_role-Verdrahtung → Dedup ODER DROP).
- **Scope-Out:** DROP von `refund_wildcards_on_leave` (toter Orphan) = separater Dead-GC-Entscheid (CEO; REVOKE schließt Leck jetzt) → D-34. W0-Rest (27 anon-SECDEF + D-12 + Hygiene) + INV-19/32/33 (P2) eigene Slices.
...
- **Fix (live, Migration `20260629200000`):** reine Subtraktion — 5 DROPs (3 Fns + 2 Tabellen). Frontend-Cleanup: `scoutMissions.ts` gelöscht + misc.ts/index.ts/keys.ts/db-invariants gescrubbt. **`qk.missions.scout` BEHALTEN** (geteilt mit lebendem `useMissionHints`), nur `qk.missions.progress` raus. **i18n (Reviewer-Catch):** 6 tote Keys (scoutMissions/scoutMissionReward/RewardBody ×DE+TR) entfernt — VOR Entfernen DB-Check `notifications` = 0 Rows (toter RPC emittierte nie → keine dynamische Auflösung).
- **Proof:** force-rollback-Smoke (5 DROPs fehlerfrei, Survivor-Gegenprobe soft_reset_season/scout_scores/mission_definitions/user_missions); post-apply AC1 5 weg + AC2 lebende da; tsc Exit 0; **db-invariants identische Failure-Menge vor/nach (6 INV pre-existing, 0 scout_mission-Erwähnung)** = 0 Regression; grep src/+messages/ = 0 verbliebene Refs; JSON valide DE/TR-Parität 63/63. Proof `458-dead-feature-gc.txt`, Review `458-review.md`.
- **Reviewer (CONCERNS→behoben):** „ein Senior merged das" — chirurgisch, geteilter Key korrekt erhalten, db-invariants präzise, 0 Consumer übersehen; einziges Finding (tote i18n-Keys) im Slice nachgezogen.
- **Knowledge:** errors-infra **S457/S458** — Dead-Feature-GC hat i18n + DB-Objekte als eigene Streich-Achsen (für tsc/src-grep unsichtbar): `grep <feature> messages/*.json` + Notification-Key-DB-Check + Caller-Enum pg_proc **+ pg_cron + ACL** + force-rollback-Smoke + Survivor-Gegenprobe. Disease-Register **D-13 + D-10 → geheilt**.

## 457 | 2026-06-29 | chore(db): D-11 — Dead-Scoring-Modell GC (bescout_scores + score_events + award_score_points gedroppt, live)
- Stage-Chain: SPEC → IMPACT (skipped, 0 lebende Consumer) → BUILD → PROVE (force-rollback + post-apply) → REVIEW (Cold-Context CONCERNS→Bookkeeping behoben) → CEO-Apply → LOG. **Dead-Feature-GC, §0 Subtraktion; CEO Anil W2-Wahl + „D-11-GC in jedem Fall".**
- **Befund (D87 Live, DB skzjfhvgccaeplydsunz):** Disease-Register **D-11** = totes 3./4./5. Scoring-Modell neben den zwei lebenden (scout_scores kanonisch + user_stats-Projektion S454). `bescout_scores` 0 Rows, `score_events` 0 Rows, `award_score_points(uuid,text,integer,text,uuid,text)` schreibt NUR in die 2 toten Tabellen, **0 SQL-Caller** (pg_proc-Writer-Enum S453) + **0 echte src/-Caller** (Grep: nur Cron-Step-Label-Strings + Test-Listen = False-Positives). ACL `{postgres,service_role}` (kein anon). Keine Views/inbound-FK/Trigger.
- **Fix (live, Migration `20260629190000`):** reine Subtraktion — `DROP FUNCTION award_score_points(...)` + `DROP TABLE score_events` + `DROP TABLE bescout_scores`. Keine CASCADE (Policies/FK/Indizes droppen automatisch mit der Tabelle; keine inbound Dependency). Schnitt-Regel-Scrub: `gamification.md:59` (award_score_points raus) + `db-invariants.test.ts` (bescout_scores aus EXPECTED_PUBLIC + score_events aus EXPECTED_SENSITIVE).
- **Path-2 (user_stats-Score-Spalten droppen) = CEO VERWORFEN** („Projektion behalten" — drift-sicher seit S454, register-gesegnet bewusste-zwei; Voll-Drop = Risiko an Level/Rank/Notif für 0 Korrektheits-Gewinn). D-17 final bewusste-zwei.
- **Proof:** pre-apply force-rollback-Smoke (DROP fehlerfrei, 3 Objekte im Tx weg, scout_scores/user_stats/score_history Survivor-Gegenprobe da, RAISE-Rollback); post-apply AC1 beide NULL + AC2 fn_count=0 + AC3 lebende Tabellen da. tsc Exit 0. **vitest: mein Change 0 Regression bewiesen** (0 Erwähnung der gedroppten Objekte in der gesamten Suite; die 5-6 Live-DB-Invariant-Failures = pre-existing W0-Security/Daten-Drift/Flakiness, INV-32-Map-Beweis). Proof `457-dead-scoring-gc.txt`, Review `457-review.md`.
- **Reviewer (CONCERNS→behoben):** „technisch wasserdicht, ein Senior merged die Migration so" — Caller-Completeness 0 übersehen, kein Über-DROP, greenfield-sicher. Findings = nur Tracking-Schuld (D-11-Heilmarkierung) → vor Commit abgehakt. Prozess-Mini-Lehre: Register-Heilmarkierung gehört VOR Reviewer-Dispatch.
- **Knowledge:** Disease-Register **D-11 → geheilt S457**; D-17 final bewusste-zwei (Path-2 CEO-verworfen); §5-Linse-2 award_score_points-Orphan-Beispiel gelöst.
