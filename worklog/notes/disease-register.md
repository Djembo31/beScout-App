...
| D-26c | **D-26-Rest ohne `club_id` in der Row → Select-/RPC-Change nötig (Daten-Contract, Impact-Analyse auf Consumer):** `watchlist.ts:123` · `lineups.queries.ts:161` · `offers.ts:43` · `trading.ts:410` (`getGlobalMovers`/Trending = RPC liefert nur Freitext) · `compare/page.tsx` (raw-Rows). **+ Player-Detail Cold-Load-Cache-Race (S286/D-03):** `usePlayerDetailData` useMemo läuft vor Club-Cache-ready → club+league fallen auf Fallback, recomputet nicht bei warm (betrifft Liga identisch). | konsist./architektur | S | CTO | 🟠 offen — Display/Such-Match (kein Money); Cache-Race = S286-Reaktivitäts-Signal. |
...
9. **[Konsistenz-Batch]** D-23 ✅ 467 · **D-26 ✅ 477** + **D-26b ✅ 478** (D-26c Rest offen) · **D-25 Login/Onboarding-i18n ✅ 479** · D-24 (Wording, Compliance/CEO) offen — klein, hoch-sichtbar.
