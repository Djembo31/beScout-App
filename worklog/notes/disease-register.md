...
| D-35 | **`get_club_dashboard_stats_v2(uuid)` anon-granted gibt Per-User-PII (`user_id`/`holdings_count`/`total_score`) an ausgeloggte Besucher** (identische Shape wie das in S461 gedroppte v1, RPC-Shape-Audit `proofs/007:132-133`). Carry-forward aus D-12: der DROP schloss nur Name-Enumeration, nicht die anon-Exposure. **Entscheid:** ist die Club-Dashboard-Seite öffentlich (ausgeloggt) gewollt → anon behalten (öffentliches Leaderboard); nur eingeloggt → `REVOKE anon`. Gleiche Frage für `get_club_top_scouts`/`get_monthly_liga_winners` (disease-register §2 ⚠️). | security/exposure | XS | **Sec/CEO** | 🟡 offen (W0-Hygiene) |
