...
| D-37b | **`get_sponsor_stats_summary`** (SOLE-gate `top_role='Admin'`, read-only Sponsor-Stats, tot) + **`set_club_fan_rank_thresholds`** (Sekundär-Branch: `top_role='Admin' OR club_admins(owner/admin)` → club_admins funktioniert, Platform-Override tot). Rest der `top_role='Admin'`-Dead-Source-Familie nach S464. | konsist./security | XS | **Sec/CEO** | 🟡 offen (Slice 465 — beide auf `platform_admins`, schließt die Familie vollständig; kein Money, read/config). |
