...
| D-36 | **`rpc_get_club_trading_fees` + `rpc_get_club_fan_stats` nutzen den dead `profiles.top_role='Admin'`-Branch** (0 Match in DB) für ihren Platform-Admin-Override → Platform-Admins ohne `club_admins`-Row sind für fremde Clubs ausgesperrt (fail-closed, **kein Leak**). Seit S462 sichtbar inkonsistent: im AdminRevenueTab liefert v2 (Platform-Admin via `platform_admins`) Daten, der Sibling `getClubTradingFees` RAISEt. | konsist./security | XS | **Sec/CEO** | 🟡 offen (W0-Konsistenz — Reviewer S462 „priorisieren": beide Siblings auf `platform_admins` umstellen, wie v2/get_club_balance) |
