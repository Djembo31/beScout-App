...
- **E-3-Regel-Erweiterungen** (je winziger Folge-Slice, kein Schema-Change):
  ...
  - **➡️ NÄCHSTER = Slice 392 `nation_in` + `max_per_nation`** auf `nationality_iso`: `nation_in` (Whitelist, Array-Wert `{type,values:[...]}` → Validator-Dispatch VOR numerischem `^[0-9]+$`-Guard, eigener Zweig + `CONTINUE`, Multi-Select-UI) + `max_per_nation` (Zahl, Spiegel max_per_club, Starter-only, GROUP BY nationality_iso WHERE <>''). Fail-closed bei `nationality_iso=''`.
  - offen nach 391/392: Builder „Bedingung hinzufügen ▾" + Echtzeit-Treffer-Anzeige (§3b, E-4/E-6).
