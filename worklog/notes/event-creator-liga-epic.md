...
- **E-3-Regel-Erweiterungen** (je winziger Folge-Slice, kein Schema-Change):
  ...
  - **🔴 Nationen-Regeln BLOCKIERT bis Normalisieren-Slice (S390-Fund):** `players.nationality` ist kaputt — Türkei = `Türkiye`/`Turkey`/`TR` (728 Spieler, 3 Schreibweisen), 207 leer, 168 Werte. `nation_in`/`max_per_nation` würden still falsch ausschließen/zählen. **CEO-Entscheid (Anil 2026-06-26): Normalisieren-Slice 391 ZUERST** (ISO-kanonisch + Backfill + Re-Drift-Guard, kein API-Key nötig), DANN Slice 392 `nation_in` + `max_per_nation` auf sauberen Daten.
