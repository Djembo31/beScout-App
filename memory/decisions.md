...
**Datum:** 2026-06-24 · **Status:** Aktiv (Kern decided) + Teil-OPEN (Naming/Einheiten-Vokabular/Pricing-Fairness, im Cleanup-Epic mit Anil zu ratifizieren) · **Category:** PRODUCT/COMPLIANCE · **Kontext:** Ein einziger echter E2E-Trade (Slice 365 E2E-Durchlauf, jarvis kauft 1 Card) deckte **systemischen Money-Modell-Drift** auf: 3 rivalisierende Namen für dieselbe Einheit (**BSD / $SCOUT / Credits**), ein **Faktor-100-Widerspruch** beim €-Bezug („1 $SCOUT = 1 cent = 0,01 €" in trading.md/treasury.md/D83 **vs** „1.000.000 cents = 10.000 $SCOUT" in CLAUDE.md **vs** Live-Code `centsToBsd = cents/100`), uneinheitliche **Phasen-Nummerierung** (1/3/4 in business.md vs 1/2 in treasury.md/D83), ein **CASP-Strategie-Widerspruch** (`wiki/scout-launch-strategie.md` „keine CASP-Lizenz" vs alle Phasen-Docs „Token erst nach CASP"), und `docs/CONCEPT-DPC-ECONOMY.md` ist **in sich selbst** widersprüchlich. Vollständige Inventur (~40 Stellen, 5 Kategorien): `worklog/notes/365-money-model-drift-inventory.md`. Anil-Klarstellung 2026-06-24 → **D99 ist die SSOT, an der ALLE Money-/Wording-Docs ausgerichtet werden.**
...
**OFFEN (Cleanup-Epic „Money-Modell Glattzug", mit Anil zu ratifizieren — KEIN Code/Doc-Massen-Edit ohne diese Ratifikation):**
- **Naming-Entscheidung:** user-facing **„Credits"** (neutral, **Empfehlung CTO** — Compliance + scharfe Phasen-Linie) **vs** überall **„$SCOUT"** (Anils Bauchgefühl). Plus: Legacy **„BSD" deprecaten**, Code-Vokabular (`centsToBsd`/`bsdToCents`/`formatScout`/`fmtScout`, JSDoc „cents → $SCOUT") vereinheitlichen.
- **Einheiten-Vokabular:** intern = **cent** (integer, Präzision), Anzeige = **Credits** (= cents/100). Die Doc-Zeile „1 $SCOUT = 1 cent" (= internes cent) widerspricht dem Code „$SCOUT = Anzeige (=100 cents)" → **Faktor-100-Namensdrift**, eindeutig auflösen.
- **Phasen-Nummerierung** vereinheitlichen: 1/3/4 (business.md ADR-028) vs 1/2 (treasury.md/D83).
- **CASP-Strategie** klären: `wiki/scout-launch-strategie.md` (Token ohne CASP, nur MiCA Title II / NCA-Notification) vs alle Phasen-Docs („Token erst nach CASP").
- **Card-Pricing-Fairness:** Anzeige-Formel `MV/1.000` (live, top-Spieler korrekt) vs Tokenisierungs-Fairness „10 % / 10.000 Cards = MV/100.000 €" → **100× auseinander**. Für jetzt nur **Spielzahl** (egal, da kein €-Wert); echte ICO-Ökonomie später festlegen. Separat: **Data-Drift** — `ipo_price` wird bei Launch eingefroren, nicht bei MV-Änderung nachgezogen → Nicht-Top-Spieler verzerrt (Douglas Willian MV 500K steht bei 10 statt ~500 Credits).

**Auswirkung:** D99 ist die **einzige Wahrheit** zum Money-Modell. ALLE in der Inventur gelisteten Stellen werden im Cleanup-Epic darauf ausgerichtet (verbunden, kein Parallel-Stand). Bis dahin: bei jeder Money-/Wording-Frage **D99 zuerst**.

**Re-Visit-Trigger:** ICO-Vorbereitung (Naming-Unifikation zum Coin, €-Peg-Festlegung, Pricing-Fairness-Ökonomie, KYC/Geo fürs Bonus-Einlösen) — und die Ratifikation der OFFEN-Punkte durch Anil zu Beginn des Cleanup-Epics.
