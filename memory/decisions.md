...
**Datum:** 2026-06-26 · **Status:** 🟢 Aktiv — **V3 (gebaut in Slice 396, supersedes V1/V2-Alignment)** · **Category:** PRODUCT + ARCHITECTURE (Events/Money) · **Kontext:** E-4 (User-Events) Alignment mit Anil. Ein normaler Nutzer soll echte Fantasy-Events erstellen können (heute `CreateEventModal` = Mock-Toast). Money/CEO-Scope (§3). Präzisiert D104 (Creator-Oberbegriff) für den Creator-Typ „User".

> **⚠️ V3-KORREKTUR (Anil, beim BUILD von 396):** Das ursprüngliche Alignment (V1/V2 unten) mit **Ersteller-Seed/Start-Pot** wurde von Anil als „Schrott" verworfen („der Ersteller legt einen Pot vor und wartet → hat nix"). **Finales gebautes Modell V3:** (a) **KEIN Seed/Start-Pot** — der Ersteller legt keinen Pot vor. (b) Der **Pot entsteht ausschließlich aus den Teilnehmer-Eintritten** (`ticket_cost`, Ersteller-wählbar ≥0). (c) **Kein Plattform-/Eintritts-Schnitt** → `event_fee_config('user') = platform 0 / beneficiary 0` (Eintritt 100 % → Pot). (d) **BeScout verdient ausschließlich über die Erstell-Gebühr** (`platform_event_config.user_event_create_fee_cents`, Default **5000 cents = 50 Cr**, admin-steuerbar via `set_user_event_create_fee`). (e) Spielt der Ersteller mit, zahlt er den Eintritt wie alle. (f) `scout_events_enabled` global an (B1). Die folgenden V1/V2-Punkte 2-6 gelten unverändert; nur Punkt 1 (Geld-Modell) ist durch V3 ersetzt.
...
### E-4b-Realisierung (Slices 397/398, 2026-06-26) — UI-Verkabelung + 3 CEO-Entscheide
Der E-4a-Geldkern war gebaut, aber **0 UI-Konsumenten** (`CreateEventModal` blieb Mock) → toter Geldkern. **Slice 397** verkabelte: echter Builder → `create_user_event` (Service `createUserEvent` + Hook `useCreateUserEvent`, S371-Wallet-Invalidate), Typ-Union `'user'` durch 5 Lookups, `errorMessages` 11 Reject-Codes (S393), i18n DE/TR. **Live bewiesen** (bescout.net): Wallet −50 Cr / Topf +50 Cr (Zero-Sum), Header sofort, entry_fee in cents (kein ×100-Bug). 3 CEO-Entscheide (AskUserQuestion):
1. **Credit-Eintritt entkoppelt von `PAID_FANTASY_ENABLED` + sichtbar (Compliance-Stance, präzedenzbildend):** `JoinConfirmDialog` zeigt den Scout-Eintritt für `type==='user'` IMMER (`event.type === 'user' || PAID_FANTASY_ENABLED`), NICHT durch globalen Flag-Flip. **Begründung:** User-Events = **Phase-1-Credits (D99, wertloses Spielgeld)** ≠ **Phase-3-Paid-Fantasy (Echtgeld, gesperrt, business.md „NICHT BAUEN")**. Künftige user-facing Credit-belastende Fantasy-Features folgen diesem Muster (gezielt `type`-verankert entkoppeln statt Flag global flippen). `FantasyDisclaimer` bleibt auf jedem Money-Schritt.
2. **Jeder eingeloggte User darf erstellen** (`FantasyHeader` isAdmin-Gate entfernt); Misuse-Schutz = Erstell-Gebühr + `min_entries`.
3. **Split** 397 (Erstellen+Eintritt) / **399** (Discovery + Live-Pot-Vorschau + Cancel-UI + Admin-Gebühr-Slider + F2/F3-Fix).
