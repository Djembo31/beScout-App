...
- **⚠️ E4 Money-Modell-Glattzug + Mock→Pro-E2E-Härtung (D99) — IN ARBEIT (Anil-Prio 2026-06-24).** Plan-Anker `worklog/notes/366-e4-money-model-cleanup-epic.md`.
  ...
  - **Schritt 3: E2E-Sweep + Bug-Fixes** (`worklog/notes/365-e2e-findings.md`): ✅ **367 T-3 Diamond-Hands** (Rename „Treuer Sammler" + Hold-Logik holdings.created_at statt Mock-Seed + Konfetti-Gate; Reviewer PASS, Commit 7b650a4f).
    - **368 — Scout-Card-Wertanzeige (REFRAMED 2026-06-24!):** Die alte P-4-Prämisse („ipo_price → MV/10 nachziehen") war FALSCH → **verworfen (D100 supersedes D99 Pkt 4).** `ipo_price` = Vereins-Eintrittspreis, MV-entkoppelt; „Drift" ist KEIN Bug, kein Daten-UPDATE. Spec `docs/plans/2026-06-24-scout-card-value-model-spec.md`. Drei Sub-Slices:
      ...
      - **← NÄCHSTER: 368b** — Anzeige-Wahrheit (UI): „Dein Einstieg" ← `ipos.price` der Erst-IPO / „—"; ipoPrice/MV/Marktpreis klar trennen + Labels vereinheitlichen („Floor"/„Marktpreis"/„Markt Floor"); CSF-Reward-Basis korrekt. Hauptfiles: `RewardsTab.tsx`, `PlayerHero.tsx`, `TradingTab.tsx`. **Open-Q:** „—"-Fall = nur Einstieg-Feld ausblenden oder ganzen Reward-Block?
      - **368c** — Floor = transparentes Orderbuch (Quelle sichtbar: offene Order vs. letzter Verkauf) + Anti-Manipulation. **Open-Q (Anil):** Anti-Manip-Regel (Mindest-Order-Größe vs. %-Schwelle vs. Median-Filter).
