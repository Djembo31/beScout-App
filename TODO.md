...
## 🔴 P0 — jetzt
- **E3 Plattform-Treasury (D96), Money/CEO-Scope, selbst bauen (§3) — IM BAU:** ✅ **Slice 1 Topf-Fundament (357)** + ✅ **Slice 2 Fees REIN Trading (358)** + ✅ **2b Fees REIN IPO (360)** + ✅ **2c Fees REIN Polls (363)** + ✅ **2d Fees REIN Research (364)** — `buy_player_sc`+`buy_from_order`→'trading', `accept_offer`→'p2p', `buy_from_ipo`→'ipo', `cast_community_poll_vote`→'poll' (beide source-Branches club+user), `unlock_research`→'research', inline `book_platform_treasury`, **voller Auffang 100 % (D98, CEO-approved)**, Zero-Sum live bewiesen. **← NÄCHSTER: letzte Fee-Quelle REIN** (Bounty `approve_bounty_submission`→'bounty' [Differenz reward−creator_net, heute nicht notiert — Body genau lesen, evtl. erst berechnen]). Dann 3 Monats-Liga e2e → 4 BeScout-Events → 5 Wettkampf-Darstellung. **Plan-Anker: `worklog/notes/358-platform-treasury-epic.md`** (+ WARUM D96/D98 + WIE treasury.md §10). Money-Muster: Live-`pg_get_functiondef` der Fee-RPCs VOR Spec (D87).
