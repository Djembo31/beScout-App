...
## 🔴 P0 — jetzt
- **E3 Plattform-Treasury (D96), Money/CEO-Scope, selbst bauen (§3) — IM BAU:** ✅ **Slice 1 Topf-Fundament (357)** — `platform_treasury` Singleton-Lock + `platform_treasury_ledger` append-only + `book_platform_treasury()`/`get_platform_balance()`/`get_platform_treasury_ledger()` + AdminTreasuryTab „Plattform-Topf"-Card, Single-Pot/Variante A, Topf live bei 0 (kein Backfill). **← NÄCHSTER: Slice 2 = Fees REIN** (eine Quelle/Slice, **Trading zuerst** via `buy_player_sc`+`accept_offer` → `book_platform_treasury('credit','trading',platform_fee,...)`); **CEO-Frage Slice 2:** voller Auffang vs. Teil-Burn/Cap (ADR-026). Dann 3 Monats-Liga e2e → 4 BeScout-Events → 5 Wettkampf-Darstellung. **Plan-Anker: `worklog/notes/358-platform-treasury-epic.md`** (+ WARUM D96 + WIE treasury.md §10). Money-Muster: Live-`pg_get_functiondef` der Fee-RPCs VOR Spec (D87).
