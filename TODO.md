...
## 🔴 P0 — jetzt
- **E3 Plattform-Treasury (D96), Money/CEO-Scope, selbst bauen (§3) — IM BAU:** ✅ **Slice 1 Topf-Fundament (357)** + ✅ **Slice 2 Fees REIN Trading (358)** — `buy_player_sc`+`buy_from_order`→source 'trading', `accept_offer`→'p2p', inline `book_platform_treasury`, **voller Auffang 100 % (D98, CEO-approved)**, Zero-Sum live bewiesen (Topf +350/+350/+200). **← NÄCHSTER: restliche Fee-Quellen REIN** (IPO `buy_from_ipo`→'ipo' · Polls `cast_community_poll_vote`→'poll' · Research→'research' · Bounty→'bounty' — je eigener Slice, gleiches Inline-Muster). Dann 3 Monats-Liga e2e → 4 BeScout-Events → 5 Wettkampf-Darstellung. **Plan-Anker: `worklog/notes/358-platform-treasury-epic.md`** (+ WARUM D96/D98 + WIE treasury.md §10). Money-Muster: Live-`pg_get_functiondef` der Fee-RPCs VOR Spec (D87).
- **🐞 Backlog (pre-existing, aus 358-Smoke): `accept_offer` side='sell' kaputt** — `type='offer_buy'` fehlt im `transactions_type_check` → `23514` bei jedem Annehmen eines Sell-Angebots (Live `offer_buy`-Count=0). S330-CHECK-Drift-Klasse. Eigener kleiner Slice: `offer_buy` in CHECK + 4-File-Sync (activityHelpers + de/tr.json). Money-nah (P2P), unabhängig von E3.
