...
| Aufstellungs-Regeln | lineup_rules (JSONB) | save_lineup generischer Validator (Weg B/D107: fail-closed bei unbekanntem type, Wert-Bound PRO TYP, VOR INSERT+Wildcard-Move) | Builder-Feld je Regel | **S385/S386/S388/S389/S390/S392** |
...
**Zwei-Töpfe-Architektur (D107):** *Eintritts-Türsteher* (wer darf rein: Abo/Stufe/Follower/Fan-Rang) = feste Spalten in `lock_event_entry`. *Aufstellungs-Regeln* (welche Karten ins Lineup) = JSONB-Regel-Liste `events.lineup_rules` + EIN generischer Validator in `rpc_save_lineup` (Weg B). Neue Regel-Art = nur neuer CASE-Zweig im Validator + eine Builder-Zeile, **kein Schema-Change**. Regeln: (1) `min_per_own_club` S385 (feste Zahl, zählt **Starter** aus `events.club_id`, fail-closed bei club NULL, Bound 1..11); (2) `age_max`/`age_min` S386 (jeder aufgestellte Spieler **Starter + Bank** muss `players.age <= / >= N`, fail-closed bei age NULL, Bound 14..50); (3) `min_per_position` S388 (`{type,position,value}`: mind. N **Starter** mit `players.position = GK/DEF/MID/ATT`, Bound 1..11 — Formations-Steuerung, z. B. „min. 3 ATT" = Angriffs-Event); (4) `mv_max_eur` S389 (jede aufgestellte Karte **Starter + Bank** `players.market_value_eur <= N EUR`, Underdog-Events; **MV=0/NULL fail-closed** = CEO-Entscheid Anil, kein versteckter Star — Trade-off: 491 echte MV=0-Jugendspieler ausgeschlossen, Backlog Re-Scrape; Admin gibt **Mio. €** ein, DB speichert EUR-Ganzzahl `Math.round(×1e6)`; Bound 1..1e9); (5) `max_per_position` S390 (`{type,position,value}`: höchstens N **Starter** je Position, Bound 1..11 — Spiegel von min_per_position, z. B. „max. 2 ATT" = defensives Event; teilt sich den Positions-Zweig mit min_per_position, getrennte `<`/`>`-Vergleiche); (6) `mv_min_eur` S390 (jede aufgestellte Karte **Starter + Bank** `market_value_eur >= N EUR`, Star-Event, Spiegel mv_max_eur; **MV=0/NULL fail-closed**, Mio→EUR, Bound 1..1e9). **Wert-Bound liegt PRO REGELTYP** (S386: globaler 1..11 war Fundament-Bug, blockte jede nicht-Zähl-Regel). **`v_rule_value` ist `BIGINT`** (S389: EUR-Werte können `::INT`-Max 2,1 Mrd übersteigen → Cast-Crash statt sauberem Reject; BIGINT + Bound eliminiert die Overflow-Klasse für alle Großwert-Regeln). **Scope-Divergenz beachten:** min/max_per_own_club + min/max_per_position = **Starter-only** (Soll-Komposition); age + mv_max_eur + mv_min_eur = **Starter + Bank** (Per-Spieler-Eignung, Auto-Sub-Schutz). **min/max_per_position zählt nach `players.position`, NICHT nach Slot** (Startelf-Slots sind server-seitig nicht positions-validiert → ein ATT-Spieler im DEF-Slot zählt als ATT). **Nationen-Regeln nutzen `players.nationality_iso`, NICHT `nationality`** (S391): Roh-`nationality` war nicht regel-tauglich (Türkei=Türkiye/Turkey/TR/Türkei=4 Schreibweisen→762 gespalten, 207 leer). Slice 391 legte kanonische generierte Spalte `nationality_iso` an (ISO-Code via `normalize_nationality()` IMMUTABLE, zero-drift). **(7) `nation_in` S392** (`{type,values:[ISO,…]}`: jede aufgestellte Karte **Starter + Bank** muss `players.nationality_iso = ANY(values)` sein, fail-closed bei `''` → reject `nation_not_allowed`; **erster Nicht-Zahl-Regeltyp** → eigener Validator-Zweig mit `CONTINUE` VOR dem numerischen `^[0-9]+$`-Guard, sonst crasht der `::BIGINT`-Cast am Array; Element-Sanity 2..6 Zeichen; Builder = durchsuchbarer Multi-Select `NationMultiSelect`, Optionen = feste kuratierte `FOOTBALL_NATIONS` [CEO-Entscheid Anil, daten-informiert n≥10 + bekannte, ~61 Codes], Namen via `Intl.DisplayNames` + Override GB-Subdivisionen/XK); **(8) `max_per_nation` S392** (höchstens N **Starter** gleicher `nationality_iso`, Bound 1..11 — Spiegel max_per_club, `GROUP BY nationality_iso WHERE <> ''` → leere ISO erzeugt KEINE Diversitäts-Gruppe; numerischer Pfad). Beide nutzen `players.nationality_iso` (S391), NIE Roh-`nationality`. **Reject-Anzeige (S393):** alle 9 Reject-Codes (`min_per_own_club_not_met`, `age_max/min`, `min/max_per_position`, `mv_max/min`, `nation_not_allowed`, `max_per_nation_exceeded`) sind seit Slice 393 **regel-spezifisch user-facing** (DE/TR im `errors`-Namespace via `KNOWN_KEYS`-Passthrough) — davor fielen alle still auf `'generic'`. **Vervollständigt S395:** die restlichen 22 `rpc_save_lineup`-Codes (Entry-State/Formation/Salary/Wildcard/Bench) sind jetzt ebenfalls regel-spezifisch — 11 Passthrough-Keys + 3 gruppierte ERROR_MAP-Regex (`lineupFormationInvalid`/`wildcardSlotInvalid`/`lineupRuleInvalid`, fasst strukturell gleiche Codes) + 2 Reuse (`auth_mismatch`→`permissionDenied`, `insufficient_sc`→`notEnoughDpc`). **`rpc_save_lineup`-Reject-Coverage damit komplett** (nur dynamischer Kontext im Toast bleibt Folge-Slice). Muster: Code==Key → `KNOWN_KEYS`-Passthrough (immun gegen Regex-Order), sonst Regex; bei jeder neuen reject-code-RPC die `'error'`-Literale gegen `KNOWN_KEYS ∪ ERROR_MAP` + `errors`-NS (DE+TR) diffen.
