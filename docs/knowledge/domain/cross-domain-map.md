...
- **Club-Treasury** (Saldo + append-only Ledger, Slice 329) ist die zentrale Geld-Drehscheibe: REIN (Trading 1% · IPO 85% · P2P 0,5% · Abo 100% · Poll-Revenue) → RAUS (CSF an Holder ✅330 · Event-Prizes ✅331 · Club-Bounties ✅332 · Fan-Rewards ⬜).
- **Domain-Kopplung:** Trading/IPO/P2P → Treasury(REIN). Liquidation(CSF) → Treasury(RAUS) → Holder-Wallets. Events(type='club') → Treasury(RAUS, Escrow). Bounties(club) → Treasury(RAUS, Escrow). Polls(Vereins) → Treasury(REIN, D86, offen). Fan-Rank/Abo → Fan-Reward-Engine.
- Kanon-Details: `docs/knowledge/domain/treasury.md` + `docs/knowledge/domain/polls.md`.
