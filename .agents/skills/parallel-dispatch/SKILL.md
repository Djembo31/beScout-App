...
description: Playbook für Parallel-Agent-Orchestration mit Worktrees. Nutze bei Multi-Domain-Tasks (>3 Files cross-domain). Zeigt konkret Dispatch-Pattern für backend+frontend+test-writer+reviewer parallel. Ersetzt Solo-Codex-Sequenziell bei grossen Features.
---
...
Override via `model: opus` in `.Codex/agents/<name>/SKILL.md` frontmatter.
...
- **Parallel bei Money-Path** → Pattern ist richtig, aber Money braucht Solo-Codex mit Hypothese

## Beispiel-Session (ideal)

```
1. /spec "Neues Fantasy-Leaderboard mit Filtern"
   → Codex erstellt worklog/specs/NNN-fantasy-leaderboard.md
