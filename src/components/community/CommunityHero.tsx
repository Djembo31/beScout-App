...
import { MessageSquare, Radio, FileText } from 'lucide-react';
...
export default function CommunityHero({ onCreatePost, onCreateRumor, onCreateResearch }: CommunityHeroProps) {
      ...
      {ACTIONS.map((a, i) => {
        ...
        return (
          <button
            key={a.key}
            onClick={handlers[i]}
            className={`group relative p-4 rounded-2xl border bg-gradient-to-br ${a.color} transition-all min-h-[44px] text-left`}
          >
            <Icon className={`w-6 h-6 ${a.iconColor} mb-2`} />
            <div className="font-bold text-sm">{t(`hero.${a.key}Label`)}</div>
            <div className="text-[11px] text-white/40 mt-0.5 line-clamp-2">{t(`hero.${a.key}Desc`)}</div>
          </button>
        );
      })}
    </div>
