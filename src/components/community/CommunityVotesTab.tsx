    ...
    <Card className="overflow-hidden">
      <div className="p-4 flex items-center justify-between bg-purple-500/[0.06] border-b border-purple-500/20">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-purple-300">{tc('clubVoteLabel')}</span>
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          {vote.cost_bsd > 0 && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-gold font-bold">{formatScout(vote.cost_bsd)} bCredits</span>
            </div>
          )}
        </div>
        {hasVoted && (
          <Chip className="mt-2 bg-purple-500/15 text-purple-300 border-purple-500/25">{tc('votedLabel')}</Chip>
        )}
      </div>
    </Card>
  );
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold">{tc('votesTabTitle')}</span>
        </div>
      ...
      {communityPolls.length > 0 && clubVotes.length > 0 && (
        <div className="flex items-center gap-3 py-2">
          ...
          <span className="text-xs text-white/30 font-semibold">{tc('clubVotesDivider')}</span>
          <div className="flex-1 h-px bg-white/10" />
        </div>
      )}
      ...
      {communityPolls.length === 0 && clubVotes.length === 0 && (
        <Card className="p-12 text-center">
          ...
          <div className="text-white/30 mb-2">{tc('noVotesYet')}</div>
          <div className="text-xs text-white/20 mb-4">{tc('noVotesHint')}</div>
          <Button variant="gold" size="sm" onClick={onCreatePoll}>
            {tc('createFirstPollBtn')}
          </Button>
        </Card>
      )}
    </div>
