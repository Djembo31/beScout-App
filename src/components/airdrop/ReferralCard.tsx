...
export default function ReferralCard({ userId }: Props) {
  ...
  return (
    <Card className="p-4 bg-gold/[0.04] border-gold/15">
      ...
      {/* Referral Code */}
      <div className="flex items-center gap-2 mb-3">
        <div className="flex-1 bg-surface-base border border-white/10 rounded-lg px-3 py-2 font-mono text-sm text-gold tracking-widest select-all">
          {code}
        </div>
        <button
          onClick={handleCopy}
          className="p-2 rounded-lg bg-surface-base border border-white/10 hover:bg-white/10 transition-colors"
