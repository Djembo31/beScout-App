...
export default function SuggestedActionBanner({ action, className = '' }: SuggestedActionBannerProps) {
  return (
    <Link
      href={action.href}
      className={`flex items-center gap-3 p-3 rounded-xl border border-white/5 bg-surface-minimal hover:bg-white/[0.04] active:scale-[0.98] transition-all ${className}`}
    >
