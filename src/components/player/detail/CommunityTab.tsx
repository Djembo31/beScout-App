...
interface CommunityTabProps {
  ...
  onVotePost: (postId: string, voteType: 1 | -1) => void;
...
type FeedFilter = 'all' | 'takes' | 'rumors';
...
function CommunityTabInner({
  ...
  // Merged feed with filter
  const filteredPosts = useMemo(() => {
    if (feedFilter === 'takes') return playerPosts.filter(p => p.post_type !== 'transfer_rumor');
    if (feedFilter === 'rumors') return playerPosts.filter(p => p.post_type === 'transfer_rumor');
    return playerPosts;
  }, [playerPosts, feedFilter]);

  const takesCount = playerPosts.filter(p => p.post_type !== 'transfer_rumor').length;
  const rumorsCount = playerPosts.filter(p => p.post_type === 'transfer_rumor').length;

  // Sentiment: only meaningful with enough trades
  const recentTrades = trades.filter(tr => {
    const diff = Date.now() - new Date(tr.executed_at).getTime();
    return diff < 7 * 24 * 60 * 60 * 1000;
  });
  const showSentiment = recentTrades.length >= MIN_SENTIMENT_TRADES;

  // Activity indicator
  const postsThisWeek = playerPosts.filter(p => {
  ...
  const sharePost = async (post: PostWithAuthor) => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: 'BeScout', text, url }); } catch { /* user cancelled */ }
    } else {
      ...
      <Card key={post.id} className={cn('p-3', isRumor && 'border-red-500/20 bg-red-500/[0.02]')}>
        <div className="flex gap-2.5">
          <div className="flex flex-col items-center gap-0.5">
            <button onClick={() => onVotePost(post.id, 1)} aria-label={tc('upvoteLabel')}
            ...
            <button onClick={() => onVotePost(post.id, -1)} aria-label={tc('downvoteLabel')}
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {/* ── 1. Scout Consensus (research-based signal) ── */}
      {playerResearch.length > 0 && (
        <ScoutConsensus research={playerResearch} />
      )}

      {/* ── 2. Research Reports (highest value content) ── */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {userId && (
              <Link href="/community" className="text-xs px-3 py-1.5 rounded-lg bg-gold/10 text-gold border border-gold/20 hover:bg-gold/20 transition-colors font-medium min-h-[32px] inline-flex items-center gap-1">
      ...
      {/* ── 3. Feed (Takes + Rumors merged, with filter chips) ── */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h3 className="font-black text-lg flex items-center gap-2 text-balance">
          ...
          {userId && (
            <div className="flex items-center gap-1.5">
              ...
              <Button variant="outline" onClick={() => setShowCreateRumor(true)} className="text-xs gap-1">
                <Radio className="size-3" aria-hidden="true" /> {t('rumor')}
              </Button>
            </div>
          )}
        </div>
        ...
        {playerPosts.length > 0 && (
          <div className="flex gap-1.5">
            {([
              { id: 'all' as FeedFilter, label: t('filterAll'), count: playerPosts.length },
              { id: 'takes' as FeedFilter, label: t('filterTakes'), count: takesCount },
              { id: 'rumors' as FeedFilter, label: t('filterRumors'), count: rumorsCount },
            ]).map(f => (
              <button
                key={f.id}
      ...
      {/* ── 4. Sentiment (only if enough trades, small at bottom) ── */}
      {showSentiment && (
        <Card className="p-4">
