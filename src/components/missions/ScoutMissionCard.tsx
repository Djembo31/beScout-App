    ...
    <Card className={cn('p-4 relative overflow-hidden', isClaimed && 'opacity-60')}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-bold text-sm text-gold">{fmtScout(mission.rewardCents / 100)} bCredits</span>
          {mission.minTier && (
...
function buildCriteriaLabels(criteria: ScoutMission['criteria'], t: (key: string, values?: Record<string, string | number | Date>) => string): string[] {
  ...
  if (criteria.max_floor_price_cents) labels.push(t('criteriaFloor', { value: `${fmtScout(criteria.max_floor_price_cents / 100)} bCredits` }));
  return labels;
}
