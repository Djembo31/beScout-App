...
export const DashboardTab = ({
  ...
  const leftBoard = ['beScout', sponsorName, t('dashboard.boardLeague'), sponsorName];
  const rightBoard = [sponsorName, 'beScout', sponsorName, t('dashboard.boardPremium')];
      ...
      <Card className="p-5">
        ...
        {currentLineup && currentLineup.players.length > 0 ? (
          <div className="space-y-4">
            ...
            <div className="rounded-xl overflow-hidden border border-green-500/20 flex">
              {/* Left LED Board */}
                      ...
                      <div key={posGroup} className={`flex justify-center ${slotPlayers.length > 1 ? 'gap-6 md:gap-16' : ''}`}>
                        {slotPlayers.map(sp => (
                          <div key={sp.slotKey} className="flex flex-col items-center relative">
                            ...
                            <div className="text-xs mt-1" style={{ color: sp.score != null ? 'rgba(255,255,255,0.8)' : `${getPosAccentColor(sp.player.position)}aa` }}>
