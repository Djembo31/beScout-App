  ...
  return (
    <Modal open={open} onClose={onClose} title={t('createTitle')}>
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="createLeagueMaxMembers" className="text-sm text-white/60 mb-1 block">{t('maxLabel')}</label>
          <input
            id="createLeagueMaxMembers"
            type="number"
            inputMode="numeric"
            value={maxMembers}
      ...
      <button onClick={() => setExpanded(!expanded)} className="w-full p-4 text-left">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className="size-10 rounded-xl bg-purple-500/15 border border-purple-500/25 flex items-center justify-center shrink-0">
              <Trophy className="size-5 text-purple-400" aria-hidden="true" />
            </div>
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-white/40">
                ...
                {isCreator && <Crown className="size-3 text-gold" aria-hidden="true" />}
              </div>
            </div>
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-1.5">
            ...
            <button onClick={() => setShowCreate(true)} className="px-3 py-2 min-h-[44px] text-xs font-semibold rounded-lg bg-gold/10 border border-gold/20 text-gold hover:bg-gold/20 transition-colors active:scale-[0.95]">
              <Plus className="size-3.5 inline mr-1" aria-hidden="true" />
              {t('create')}
            </button>
          </div>
        </div>
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <Card className="p-8 text-center">
          <Trophy className="size-12 mx-auto mb-3 text-white/15" aria-hidden="true" />
