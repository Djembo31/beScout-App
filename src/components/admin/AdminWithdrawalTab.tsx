      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="p-4 bg-gold/[0.06] border-gold/20">
          ...
          {loading ? <Skeleton className="h-7 w-24" /> : (
            <div className="text-xl font-mono font-black text-gold tabular-nums">{formatScout(balance?.available ?? 0)} CR</div>
          )}
        </Card>
        <Card className="p-4">
          <div className="text-[10px] text-white/40 uppercase mb-1">{t('wdTotalEarned')}</div>
          {loading ? <Skeleton className="h-7 w-24" /> : (
            <div className="text-xl font-mono font-black tabular-nums">{formatScout(balance?.total_earned ?? 0)} CR</div>
          )}
        </Card>
        <Card className="p-4">
          <div className="text-[10px] text-white/40 uppercase mb-1">{t('wdTradingFees')}</div>
          {loading ? <Skeleton className="h-7 w-24" /> : (
            <div className="text-sm font-mono font-bold text-emerald-400 tabular-nums">{formatScout(balance?.trade_fees ?? 0)} CR</div>
          )}
        </Card>
        <Card className="p-4">
              ...
              return (
                <div key={w.id} className="flex items-center justify-between bg-surface-minimal rounded-xl px-4 py-3 border border-white/[0.06]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-mono font-bold tabular-nums">{formatScout(w.amount_cents)} CR</div>
