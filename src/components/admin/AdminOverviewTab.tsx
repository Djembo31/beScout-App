      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="p-4 bg-gold/[0.06] border-gold/20">
          ...
          {loading ? <Skeleton className="h-7 w-24" /> : (
            <div className="text-xl font-mono font-black text-gold">
              {formatScout(stats?.ipo_revenue_cents ?? 0)} <span className="text-sm text-white/50">Credits</span>
            </div>
          )}
        </Card>
        <Card className="p-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-xl font-mono font-black text-white">
            {fmtScout(totalVolume)} <span className="text-sm text-white/50">Credits</span>
          </div>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-2">
      ...
      {subData && subData.total > 0 && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <Card className="p-4 border-gold/20">
            ...
            <div className="text-xl font-mono font-black text-gold">{formatScout(subData.revenueCents)} CR</div>
          </Card>
        </div>
      )}
              ...
              {topTraded.map((p, i) => (
                <Link key={p.id} href={`/player/${p.id}`}>
                  <div className="flex items-center justify-between p-3 bg-surface-minimal rounded-xl border border-white/10 hover:border-gold/30 transition-all shadow-card-sm">
                    ...
                    <div className="font-mono text-sm text-gold">{fmtScout(centsToBsd(p.volume_24h))} CR</div>
                  </div>
                </Link>
              ))}
            </div>
