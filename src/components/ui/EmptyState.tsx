    ...
    <Card className={`p-12 text-center ${className}`}>
      ...
      {action && (
        action.href ? (
          <Link
            href={action.href}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm font-bold bg-gold/15 border border-gold/30 text-gold rounded-xl hover:bg-gold/25 transition-all"
          >
            {action.label}
          </Link>
        ) : (
