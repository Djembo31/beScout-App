...
import {
  ...
  DollarSign, ExternalLink, Loader2, Megaphone, Sparkles, Building2, Trophy, Banknote, Coins, Heart, Database, FileSpreadsheet,
} from 'lucide-react';
...
type AdminTab = 'overview' | 'users' | 'clubs' | 'founding_passes' | 'treasury' | 'fees' | 'economy' | 'liga' | 'ipos' | 'gameweeks' | 'events' | 'airdrop' | 'sponsors' | 'creator_fund' | 'wishes' | 'data_sync' | 'csv_import' | 'debug';
...
const TAB_ICONS: Record<AdminTab, React.ElementType> = {
  ...
  creator_fund: Sparkles, wishes: Heart, data_sync: Database, csv_import: FileSpreadsheet, debug: Bug,
};
const TAB_ORDER: AdminTab[] = ['overview', 'users', 'clubs', 'founding_passes', 'treasury', 'fees', 'economy', 'liga', 'ipos', 'gameweeks', 'events', 'airdrop', 'sponsors', 'creator_fund', 'wishes', 'data_sync', 'csv_import', 'debug'];
    ...
    <div className="space-y-6">
      ...
      {(() => {
        const TAB_LABELS: Record<AdminTab, string> = {
          ...
          sponsors: t('tabSponsors'), creator_fund: t('tabCreatorFund'), wishes: 'Fan Wishes', data_sync: t('tabDataSync'), csv_import: t('tabCSVImport'), debug: t('tabDebug'),
        };
        return (
      ...
      {tab === 'csv_import' && <AdminCSVImportTab />}
      {tab === 'debug' && <DebugTab />}
    </div>
  );
}
