...
export async function getAllEventsAdmin(filters?: {
  ...
  let query = supabase
    .from('events')
    .select('id, name, type, status, format, gameweek, entry_fee, ticket_cost, currency, prize_pool, max_entries, current_entries, starts_at, locks_at, ends_at, scored_at, created_by, club_id, sponsor_name, sponsor_logo, event_tier, tier_bonuses, min_tier, min_subscription_tier, requires_follow, min_fan_rank_tier, salary_cap, reward_structure, scope, lineup_size, is_liga_event, league_id, created_at, clubs(name, slug)')
    .order('created_at', { ascending: false });
