...
export async function votePost(
  ...
  //
  // Slice 180 (Tier B2): typed errors via @/lib/errors instead of raw Error
  // literal keys (fixes INV-25 regression). ConflictError replaces prior
  // raw-string throws for vote_post RPC failures.
  // Slice 165 Silent-Cast-Hardening preserved: RPC `vote_post` hat inkonsistente
  // Return-Shape — Success-Path: `{upvotes, downvotes}` (kein `success: true` flag!),
  // Error-Path: `{success: false, error: '...'}`. Pre-Cast-Guard throws ConflictError
  // bei RPC-Error statt silent-cast auf undefined-Felder.
  if (!data || typeof data !== 'object') {
    throw new UnexpectedError('vote_post returned empty body');
  }
  ...
  if (result.success === false || typeof result.upvotes !== 'number') {
    throw new ConflictError(result.error ?? 'vote_post_failed', 'post_vote');
  }
