...
export async function getPlayerById(id: string): Promise<DbPlayer | null> {
  const { data, error } = await supabase
    .from('players')
    .select('id, first_name, last_name, position, club, club_id, age, shirt_number, nationality, image_url, matches, goals, assists, clean_sheets, perf_l5, perf_l15, perf_season, dpc_total, dpc_available, floor_price, last_price, ipo_price, price_change_24h, volume_24h, status, market_value_eur, success_fee_cap_cents, max_supply, is_liquidated, last_appearance_gw, created_at, updated_at')
    .eq('id', id)
    .single();
...
export function dbToPlayer(db: DbPlayer): Player {
  ...
  return {
    ...
    gwGap: 0, // Calculated at query level when current GW is known
  };
}
