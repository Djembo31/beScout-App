...
export async function getBountiesByClub(
  ...
  const { data, error } = await supabase
    .from('bounties')
    .select('id, club_id, club_name, created_by, title, description, reward_cents, deadline_at, max_submissions, player_id, position, status, submission_count, min_tier, type, fixture_id, is_user_bounty, treasury_escrowed, created_at, updated_at')
    .eq('club_id', clubId)
...
export async function createUserBounty(params: {
  ...
  // Throws i18n-keys (see src/lib/errorMessages.ts KNOWN_KEYS) — consumer must resolve via t('errors.KEY').
  // Rationale: no dynamic values in error-messages, no tickers, no DE/EN mix (common-errors.md J3 Healer A).
  if (params.rewardCents < 100) throw new Error('bountyRewardMinimum');
  if (params.rewardCents > 100_000_000) throw new Error('bountyRewardMaximum');
  if (params.deadlineDays < 1 || params.deadlineDays > 90) throw new Error('bountyDeadlineInvalid');
  if (params.maxSubmissions < 1 || params.maxSubmissions > 100) throw new Error('bountyMaxSubmissionsInvalid');
  if (!params.title.trim()) throw new Error('bountyTitleRequired');
  if (!params.description.trim()) throw new Error('bountyDescriptionRequired');
