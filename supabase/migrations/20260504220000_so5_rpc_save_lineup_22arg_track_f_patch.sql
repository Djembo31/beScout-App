-- =============================================================================
-- Slice SO-5 (2026-05-04) — Wildcard-RPC-Drift-Heal post-Migration #2 #3
--
-- Drift-Quelle: Slice 251 Wave 2 (28.04) wrote 17-arg rpc_save_lineup,
-- aber Slice 195d (25.04) hatte parallel 22-arg rpc_save_lineup mit
-- Bench-Slots eingeführt. Slice 251 Wave 2 wurde NIE applied bis SO-5.
--
-- Nach Apply von Migration 20260428120500_wildcards_rpcs_per_league.sql
-- (DROP alte 5-arg spend_wildcards, CREATE neue 6-arg) waren die alten
-- Calls aus 22-arg rpc_save_lineup broken (calls vanished signature).
--
-- Dieser Patch:
--   1. DROP'pt die 17-arg rpc_save_lineup (dead code aus Slice 251 Wave 2)
--   2. CREATE OR REPLACE 22-arg mit Track F (v_event_league_id Lookup +
--      6-arg spend_wildcards / earn_wildcards mit p_league_id)
--
-- Source-of-truth: 22-arg Body aus pg_get_functiondef Slice 195d Pre-SO-5
-- + Section 14 (wildcard delta) ersetzt durch Track F Pattern aus
-- Slice 251 Wave 2 Track F save_lineup_per_league.sql.
--
-- Applied 2026-05-04 via mcp__supabase__apply_migration mit Anil-go.
-- =============================================================================

DROP FUNCTION IF EXISTS public.rpc_save_lineup(
