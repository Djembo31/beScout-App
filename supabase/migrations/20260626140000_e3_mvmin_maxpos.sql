...
-- CREATE OR REPLACE gegen Live-Baseline (Post-389, D87). Nur Validator-Block:
--   (1) Whitelist += 'max_per_position','mv_min_eur'.
--   (2) Branch max_per_position (Spiegel min_per_position 388: Starter-only, players.position, reject > value).
--   (3) Branch mv_min_eur (Spiegel mv_max_eur 389: Starter+Bank, fail-closed MV=0/NULL, reject < value).
-- Keine neuen DECLARE (v_rule_position/v_pos_count/v_player_mv existieren; v_rule_value ist BIGINT seit 389).
-- Alle Nicht-Validator-Blöcke byte-identisch (PATCH-AUDIT).
...
BEGIN
  ...
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      IF v_rule_type IS NULL OR v_rule_type NOT IN ('min_per_own_club','age_max','age_min','min_per_position','mv_max_eur','max_per_position','mv_min_eur') THEN
      ...
      ELSIF v_rule_type = 'mv_max_eur' THEN
        -- Slice 389 (E-3): Marktwert-Deckel pro Karte (EUR). Bound 1..1e9 (BIGINT, kein ::INT-Overflow).
        -- Starter + Bank (Eignungs-Regel wie Alter). Fail-closed bei MV=0/NULL (CEO-Entscheid Anil).
      ...
      ELSIF v_rule_type = 'mv_min_eur' THEN
        -- Slice 390 (E-3): Marktwert-Mindestwert pro Karte (Star-Event, Spiegel mv_max_eur).
        -- Bound 1..1e9 (BIGINT). Starter + Bank. Fail-closed bei MV=0/NULL (unbekannter Wert kann Floor nicht beweisen).
        IF v_rule_value < 1 OR v_rule_value > 1000000000 THEN
          RETURN jsonb_build_object('ok', false, 'error', 'invalid_lineup_rule_value', 'rule', v_rule_type);
        END IF;
        FOR v_i IN 1..12 LOOP
