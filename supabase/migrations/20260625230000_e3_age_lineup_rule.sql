-- Slice 386 (E-3, D107) — Alters-Fenster als Aufstellungs-Regel.
-- KEIN Schema-Change (Weg B): age_min/age_max sind neue Einträge in der bestehenden
-- events.lineup_rules-JSONB-Liste. rpc_save_lineup CREATE OR REPLACE gegen Live-Baseline
-- (pg_get_functiondef, D87). Geändert ggü. Slice 385: NUR (a) DECLARE v_player_age,
-- (b) der generische Validator-Block — Whitelist erweitert, Wert-Bound von global 1..11
-- auf PRO REGELTYP gezogen (Fundament-Fix: 1..11 galt fälschlich für alle Typen),
-- age_max/age_min-Branches (Starter + Bank, fail-closed bei age IS NULL).
-- Alle übrigen Blöcke byte-identisch (PATCH-AUDIT).
...
DECLARE
  ...
  v_player_age INT;  -- E-3 (Slice 386) addition — Alters-Regel
BEGIN
  ...
  -- bei Reject wird KEINE Ressource bewegt.
  -- Slice 386: + age_max/age_min. Wert-Bound jetzt PRO REGELTYP (vorher global 1..11,
  -- was nur zu Zähl-Regeln passte — Fundament-Fix für alle Folge-Regeln).
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      IF v_rule_type IS NULL OR v_rule_type NOT IN ('min_per_own_club','age_max','age_min') THEN
        RETURN jsonb_build_object('ok', false, 'error', 'unknown_lineup_rule', 'rule', v_rule_type);
      END IF;

      -- Globaler Wert-Guard (alle Regeln = nicht-negative Integer); Bereichs-Bound pro Typ.
      ...
      ELSIF v_rule_type IN ('age_max','age_min') THEN
        -- Alters-Bound = saner Fußball-Range (Anti-Müll); echte Filter-Zahl wählt der Admin.
        IF v_rule_value < 14 OR v_rule_value > 50 THEN
          RETURN jsonb_build_object('ok', false, 'error', 'invalid_lineup_rule_value', 'rule', v_rule_type);
        END IF;
        -- Per-Spieler-Eignung gilt für Starter UND Bank (Auto-Sub kann Bank einwechseln);
        -- fail-closed bei age IS NULL. Scope-Spiegel der E-1-Liga-Bindung.
        FOR v_i IN 1..12 LOOP
          v_pid := v_all_slots[v_i];
          IF v_pid IS NOT NULL THEN
