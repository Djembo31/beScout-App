-- Slice 406 — Club-Treasury Single-Source-of-Truth (Welle 1.3 Mock→Pro)
--
-- Problem: Der Club-Anteil jedes Kaufs wurde in ZWEI Konten geschrieben:
--   (1) Ledger club_treasury_ledger via Trigger trg_trades_book_club_treasury (KANONISCH,
--       von get_club_balance/request_club_withdrawal gelesen),
--   (2) Counter clubs.treasury_balance_cents direkt im RPC-Body.
-- Live-Befund (2026-06-27): den Counter liest KEINE Funktion und keine UI (4 Writer, 0 Reader),
-- er ist von der kanonischen Quelle gedriftet (bis 5.715 Credits über 21 Clubs).
-- → Counter-Writes aus allen 4 Kauf-RPCs entfernen + Spalte droppen. Eine Quelle: der Ledger.
--
-- Geldneutral: jede der 4 RPCs INSERTet eine trades-Zeile mit club_fee/club_share → der Trigger
-- bucht den Club-Anteil ohnehin in den Ledger. Das Entfernen des Counter-Writes ändert am
-- Auszahlungs-/Ledger-Pfad NICHTS (force-rollback Zero-Sum in 406-money-smoke.txt bewiesen).
--
-- Bodies 1:1 aus Live-pg_get_functiondef (D87) übernommen; NUR der treasury_balance_cents-Block
-- ersetzt (PATCH-AUDIT S156: alle Guards/Idempotency/fee_config/book_platform_treasury/credit_pbt
-- unverändert). Signaturen unverändert → CREATE OR REPLACE erhält ACL (S368c, kein REVOKE/GRANT nötig).
...
-- 4) buy_from_ipo
-- ============================================================
CREATE OR REPLACE FUNCTION public.buy_from_ipo(p_user_id uuid, p_ipo_id uuid, p_quantity integer, p_idempotency_key text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- [Slice 406] Counter-Write clubs.treasury_balance_cents entfernt (write-only Orphan);
  -- Club-Anteil (85% IPO) wird kanonisch via Trigger trg_trades_book_club_treasury (trades-INSERT oben) gebucht.
...
-- ============================================================
-- 5) Spalte droppen (write-only Orphan, 0 Reader)
-- ============================================================
ALTER TABLE public.clubs DROP COLUMN IF EXISTS treasury_balance_cents;
