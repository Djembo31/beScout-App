...
-- ============================================
-- Previously inlined PBT credit logic (INSERT INTO pbt_treasury + pbt_transactions).
-- buy_from_order already calls credit_pbt(). This unifies both RPCs.
-- Only change: replace inline PBT block with PERFORM credit_pbt(), remove v_pbt_balance.
...
BEGIN
  ...
  IF p_quantity != 1 THEN RETURN json_build_object('success', false, 'error', 'Im Pilot nur 1 DPC pro Kauf'); END IF;
  ...
  IF v_circular_count > 0 THEN RETURN json_build_object('success', false, 'error', 'Verdaechtiges Handelsmuster.'); END IF;
  ...
  INSERT INTO transactions (user_id, type, amount, balance_after, reference_id, description) VALUES
    (p_user_id, 'buy', -v_total_cost, v_new_balance, v_trade_id, format('%s DPC @ %s Cents', p_quantity, v_order.price)),
    (v_order.user_id, 'sell', v_seller_proceeds, v_seller_balance, v_trade_id, format('%s DPC verkauft (-%s Gebuehr)', p_quantity, v_total_fee));
