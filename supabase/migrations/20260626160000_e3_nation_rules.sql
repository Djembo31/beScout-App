-- Slice 392 — E-3 Nationen-Regeln: nation_in (Länder-Whitelist) + max_per_nation
-- Additiv zu rpc_save_lineup (Post-391-Baseline, D107 Topf 2, Weg B).
-- Drei additive Änderungen (Rest byte-identisch, PATCH-AUDIT):
--   (1) DECLARE: v_nation_values / v_player_nat / v_nat_count
--   (2) Whitelist NOT IN (...) += 'nation_in','max_per_nation'
--   (3) nation_in-Zweig DIREKT nach unknown-Check, VOR dem numerischen
--       Guard ('^[0-9]+$' / ::BIGINT), endet mit CONTINUE — weil nation_in
--       statt 'value' (Skalar) ein 'values' (Array) hat und am numerischen
--       Guard sonst crashen/falsch-rejecten würde.
--   (4) max_per_nation-ELSIF (numerischer Pfad, Spiegel max_per_club, Starter-only).
-- Fail-closed: nation_in lehnt Spieler mit nationality_iso = '' ab.
...
CREATE OR REPLACE FUNCTION public.rpc_save_lineup(
  ...
  p_slot_gk uuid DEFAULT NULL::uuid, p_slot_def1 uuid DEFAULT NULL::uuid,
  p_slot_def2 uuid DEFAULT NULL::uuid, p_slot_def3 uuid DEFAULT NULL::uuid,
  p_slot_def4 uuid DEFAULT NULL::uuid, p_slot_mid1 uuid DEFAULT NULL::uuid,
  p_slot_mid2 uuid DEFAULT NULL::uuid, p_slot_mid3 uuid DEFAULT NULL::uuid,
  p_slot_mid4 uuid DEFAULT NULL::uuid, p_slot_att uuid DEFAULT NULL::uuid,
  p_slot_att2 uuid DEFAULT NULL::uuid, p_slot_att3 uuid DEFAULT NULL::uuid,
  p_bench_gk uuid DEFAULT NULL::uuid, p_bench_o1 uuid DEFAULT NULL::uuid,
  p_bench_o2 uuid DEFAULT NULL::uuid, p_bench_o3 uuid DEFAULT NULL::uuid,
  p_bench_order integer[] DEFAULT ARRAY[1, 2, 3])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      -- nation_in: Array-Wert ('values'), KEIN numerischer Skalar. Eigener Zweig
      -- VOR dem numerischen Guard, endet mit CONTINUE (sonst crasht ::BIGINT).
      IF v_rule_type = 'nation_in' THEN
        ...
        -- Element-Sanity (fail-closed gegen Müll-Werte): ISO-Codes 2..6 Zeichen.
        IF EXISTS (SELECT 1 FROM unnest(v_nation_values) AS e
                   WHERE char_length(e) < 2 OR char_length(e) > 6) THEN
          RETURN jsonb_build_object('ok', false, 'error', 'invalid_lineup_rule_value', 'rule', v_rule_type);
        END IF;
        -- Starter + Bank (Eignung): jeder muss in der Whitelist sein; '' fail-closed.
        FOR v_i IN 1..12 LOOP
          v_pid := v_all_slots[v_i];
          IF v_pid IS NOT NULL THEN
      ...
      ELSIF v_rule_type = 'max_per_nation' THEN
        IF v_rule_value < 1 OR v_rule_value > 11 THEN
          RETURN jsonb_build_object('ok', false, 'error', 'invalid_lineup_rule_value', 'rule', v_rule_type);
        END IF;
        -- Starter-only (Komposition, Spiegel max_per_club); leere ISO nicht gezählt.
