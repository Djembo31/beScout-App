-- Slice 388 (E-3, D107) — Min-pro-Position als Aufstellungs-Regel (Formations-Steuerung).
-- KEIN Schema-Change (Weg B): min_per_position = neuer Eintrag in events.lineup_rules.
-- rpc_save_lineup CREATE OR REPLACE gegen Live-Baseline (Post-386, D87).
-- Geändert ggü. 386: NUR (a) DECLARE v_rule_position/v_pos_count, (b) Whitelist
-- + 'min_per_position', (c) neuer Branch — zählt Starter nach players.position
-- (Startelf-Slots sind server-seitig NICHT positions-validiert → players.position
-- ist die Wahrheit), Position-Whitelist VOR Count, Bound pro Typ 1..11.
...
DECLARE
  ...
  v_rule_position TEXT; v_pos_count INT;  -- E-3 (Slice 388) addition — Min-pro-Position
BEGIN
  ...
  -- E-3 (Slice 385/386/388, D107): generischer Aufstellungs-Regel-Validator (Weg B).
  -- Fail-closed bei unbekanntem Typ. Läuft VOR INSERT/UPDATE + VOR Wildcard-Bewegung →
  -- bei Reject wird KEINE Ressource bewegt. Wert-Bound PRO REGELTYP.
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      IF v_rule_type IS NULL OR v_rule_type NOT IN ('min_per_own_club','age_max','age_min','min_per_position') THEN
      ...
      ELSIF v_rule_type = 'min_per_position' THEN
        -- Min-pro-Position (Formations-Steuerung). Zählt Starter nach players.position
        -- (Startelf-Slots sind nicht positions-validiert → players.position = Wahrheit).
        ...
        IF COALESCE(v_pos_count, 0) < v_rule_value THEN
