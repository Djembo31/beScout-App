-- =============================================================================
-- Slice 251 Wave 2 Track F — rpc_save_lineup: league-aware wildcard spend/earn
--
-- Source-of-truth: supabase/migrations/20260417110000_save_lineup_formation_validation.sql
-- Applied patches: Slice 251 Wave 2 Track F — p_league_id lookup for wildcards
--
-- PATCH-AUDIT PFLICHT (errors-db.md "CREATE OR REPLACE FUNCTION — PATCH-AUDIT"):
-- Letzter CREATE OR REPLACE: 20260417110000_save_lineup_formation_validation.sql
-- Dieser File ist die vollstaendige aktuelle Source-of-truth des rpc_save_lineup Body.
-- Preserve: auth guard, event check, status check, locks_at check, entry check,
--           B4 formation validation, duplicate guard, SC check, salary cap,
--           holding locks. Edit ONLY: Step 14 (wildcard spend/earn calls).
--
-- WICHTIG: NICHT via `mcp__supabase__apply_migration` ausfuehren —
--          Anil appliziert manuell post-Merge.
--
-- Verify-SQL post-apply:
--   SELECT pg_get_functiondef('public.rpc_save_lineup(uuid,uuid,text,text,text[],uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid)'::regprocedure)
--   ILIKE '%v_event_league_id%';
--   -- Expected: t (true) = patch applied
--
--   -- Functional smoke (check rpc_save_lineup still works):
...
DECLARE
  ...
  -- Track F addition: event league for per-league wildcard tracking
...
BEGIN
  ...
  -- 9. Wildcard event-level validation
  IF v_wildcard_count > 0 THEN
  ...
  -- 14. Wildcard spend/refund — Track F: lookup event.club.league_id for per-league tracking
  v_wildcard_delta := v_wildcard_count - v_old_wildcard_count;
  IF v_wildcard_delta != 0 THEN
    -- Lookup league_id from event → club (Track F addition)
