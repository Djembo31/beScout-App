...
-- CREATE OR REPLACE gegen Live-Baseline (Post-388, D87). Nur 3 Änderungen am Validator-Block:
--   (1) DECLARE v_rule_value INT -> BIGINT (Overflow-Fix: MV-EUR-Werte können > INT-Max);
--       neu v_player_mv INT.
--   (2) Whitelist += 'mv_max_eur'.
--   (3) Neuer Branch mv_max_eur (Starter + Bank, fail-closed bei MV=0/NULL — CEO-Entscheid Anil).
...
BEGIN
  ...
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      IF v_rule_type IS NULL OR v_rule_type NOT IN ('min_per_own_club','age_max','age_min','min_per_position','mv_max_eur') THEN
      ...
      ELSIF v_rule_type = 'mv_max_eur' THEN
        ...
        -- Starter + Bank (Eignungs-Regel wie Alter — Auto-Sub wechselt Bank ein).
        -- Fail-closed bei MV=0/NULL (CEO-Entscheid Anil: kein versteckter Star im Underdog-Event).
