...
-- D104/D105: events.league_id = Fußball-Liga (NULL = offen/alle Ligen).
-- 1) Spalte additiv (kein Backfill — bestehende Events bleiben offen).
-- 2) rpc_save_lineup: additiver Liga-Gate-Block (nur aktiv wenn events.league_id IS NOT NULL).
--    Body sonst byte-identisch zur Live-Baseline (pg_get_functiondef, 2026-06-25, D87).
--    Money/Wildcard/Salary/Holdings/max_per_club unverändert.

ALTER TABLE public.events
  ADD COLUMN IF NOT EXISTS league_id uuid NULL REFERENCES public.leagues(id);

CREATE INDEX IF NOT EXISTS idx_events_league_id ON public.events(league_id) WHERE league_id IS NOT NULL;

COMMENT ON COLUMN public.events.league_id IS
  'E-1 (D104/D105): Fußball-Liga-Bindung. NULL = offen (alle Ligen). Gesetzt = Aufstellung nur aus Vereinen dieser Liga (rpc_save_lineup-Gate). NICHT zu verwechseln mit is_liga_event (= BeScout-Saison-Wertung).';
