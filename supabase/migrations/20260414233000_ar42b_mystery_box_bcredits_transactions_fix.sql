-- =============================================================================
-- AR-42b (Operation Beta Ready, Journey #5) — Mystery Box bCredits INSERT Fix
--
-- PROBLEM (Reviewer-Pass 2026-04-14, folgt AR-42 Equipment-Fix):
--   RPC `open_mystery_box_v2` bcredits-Branch macht:
--     INSERT INTO public.transactions (user_id, type, amount_cents, description)
--   Live-Schema `public.transactions` hat:
--     id, user_id, type (NOT NULL), amount (bigint NOT NULL), balance_after
--     (bigint NOT NULL), reference_id, description, created_at
--   → `amount_cents` existiert NICHT. `balance_after` ist NOT NULL und fehlt.
--
-- EVIDENZ (Live-DB Query 2026-04-14):
--   SELECT DISTINCT type FROM public.transactions → kein `mystery_box_reward`.
--   D.h. bCredits-Drops aus Mystery Box haben NIE funktioniert, seit RPC
--   existiert. Silent rollback jedes bCredits-Rolls — User sah "Open Error".
--
-- IMPACT:
--   Bcredits-Rarity-Weights in mystery_box_config definiert (Common bis Mythic
--   haben bcredits als reward_type), aber 0 Treffer in Transactions-Tabelle.
--   Reward-Pfad 2 von 4 (Equipment + Bcredits) war tot.
--
-- FIX (analog ticket_transactions Pattern im selben RPC):
...
--
-- SECURITY: CREATE OR REPLACE resettet Grants — explizit REVOKE/GRANT (J5-Learning).
-- =============================================================================
  ...
  ELSE
    ...
    CASE v_reward_type
      ...
      WHEN 'bcredits' THEN
        ...
        -- AR-42b FIX (2026-04-14): transactions columns are `amount` + `balance_after`, NOT `amount_cents`.
        INSERT INTO public.transactions (user_id, type, amount, balance_after, description)
        VALUES (v_uid, 'mystery_box_reward', v_bcredits,
                (SELECT balance FROM public.wallets WHERE user_id = v_uid),
                v_rarity || ' Mystery Box: ' || (v_bcredits / 100) || ' CR');
...
COMMENT ON FUNCTION public.open_mystery_box_v2(boolean) IS
  'AR-42+42b (2026-04-14): Equipment INSERT uses `rank`; transactions INSERT uses `amount` + `balance_after` (not `amount_cents`). bcredits reward path was tot seit RPC existiert.';
