-- Slice 417: get_market_user_dashboard open_bids — Eigen-Gebot-Ausschluss (server-SSOT).
-- Byte-identisch zur Live-Baseline (D87/pg_get_functiondef) bis auf EINE Zeile:
--   open_bids-Subquery erhaelt `AND sender_id <> p_user_id`.
-- Grund: eigene oeffentliche Kaufgebote auf besessene Spieler sind nicht annehmbar
--   (accept_offer blockt Selbst-Annahme, S416) → duerfen nicht im "Offene Gebote"-Set
--   erscheinen. Spiegelt den Service-Fix in getOpenBids({ownedByUserId}).
-- CREATE OR REPLACE auf existierende Funktion → ACL bleibt erhalten (S368c); kein
--   Grant-Touch. Auth-Guard (auth_uid_mismatch) unveraendert. Kein Geldfluss (read-only).
