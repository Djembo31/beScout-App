-- Slice 409 — P2P-Offer Escrow-Robustheit: Refund-Symmetrie heilen [Welle 1.4c, D112]
--
-- Escrow-Modell (Live-bewiesen): create_offer(buy) verschiebt total=price*qty von
-- balance->locked. Korrektes Unlock = balance += total, locked -= total (cancel/reject
-- machen es richtig). 4 Stellen brachen das (force-rollback Zero-Sum je diff=-100):
--   (a) accept_offer buy-Fulfillment: balance -= total UND locked -= total = Doppelbelastung.
--   (b) accept_offer expired-Branch (buy): locked -= total, kein balance += (Leak).
--   (c) accept_offer insufficient-qty-Branch (buy): locked -= total, kein balance += (Leak).
--   (d) expire_pending_offers: locked -= price (statt price*qty) + kein balance += (Leak).
-- Fix: alle 4 auf das korrekte Reversal/Consume. Bodies aus Live/407-Stand (D87, PATCH-AUDIT
-- S156) — nur Escrow-Mathe geändert, Guards/Fee(350/150/100)/Routing/Idempotency byte-identisch.
...
BEGIN
  ...
  IF v_offer.expires_at IS NOT NULL AND v_offer.expires_at < now() THEN
    ...
    -- [Slice 409] buy-Offer-Refund = balance += total UND locked -= total (mirror cancel/reject)
    IF v_offer.side = 'buy' THEN
      UPDATE wallets SET balance = balance + (v_offer.price * v_offer.quantity),
                         locked_balance = GREATEST(0, locked_balance - (v_offer.price * v_offer.quantity)),
                         updated_at = now()
      WHERE user_id = v_offer.sender_id;
      INSERT INTO transactions (user_id, type, amount, balance_after, reference_id, description)
        SELECT v_offer.sender_id, 'offer_unlock', v_offer.price * v_offer.quantity, w.balance, p_offer_id, 'Angebot abgelaufen — Guthaben freigegeben'
  ...
  IF COALESCE(v_seller_qty, 0) < v_offer.quantity THEN
    ...
    IF v_offer.side = 'buy' THEN
      ...
      WHERE user_id = v_offer.sender_id;
      INSERT INTO transactions (user_id, type, amount, balance_after, reference_id, description)
        SELECT v_offer.sender_id, 'offer_unlock', v_offer.price * v_offer.quantity, w.balance, p_offer_id, 'Angebot storniert — Guthaben freigegeben'
        FROM wallets w WHERE w.user_id = v_offer.sender_id;
    END IF;
  ...
  IF v_offer.side = 'buy' THEN
    -- [Slice 409] buy-Fulfillment: Käufer-Funds liegen bereits im locked (seit create);
    -- Zahlung = Escrow KONSUMIEREN (locked -= total). balance NICHT erneut belasten (war Doppelbelastung).
    UPDATE wallets SET
...
-- ============================================================
-- expire_pending_offers — buy-Offer-Refund: balance += price*qty (mirror cancel) + offer_unlock-Tx
-- ============================================================
...
DECLARE
  ...
  v_funds_released BIGINT := 0;
  v_offer RECORD;
  v_amount BIGINT;
BEGIN
  FOR v_offer IN
    SELECT id, sender_id, price, quantity, side
    FROM offers
    WHERE status = 'pending' AND expires_at < now()
    FOR UPDATE SKIP LOCKED
  LOOP
    -- [Slice 409] buy-Offer-Refund = balance += total UND locked -= total (price*quantity, mirror cancel/reject)
    IF v_offer.side = 'buy' THEN
      v_amount := v_offer.price * v_offer.quantity;
      UPDATE wallets
      ...
      WHERE user_id = v_offer.sender_id;
      INSERT INTO transactions (user_id, type, amount, balance_after, reference_id, description)
        SELECT v_offer.sender_id, 'offer_unlock', v_amount, w.balance, v_offer.id, 'Angebot abgelaufen — Guthaben freigegeben'
        FROM wallets w WHERE w.user_id = v_offer.sender_id;
      v_funds_released := v_funds_released + v_amount;
    END IF;
  ...
  RETURN json_build_object(
    ...
    'funds_released_cents', v_funds_released
  );
END;
$function$;
