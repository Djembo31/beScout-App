...
-- Body = live pg_get_functiondef 2026-06-18 (== Migration 336, D87-verifiziert).
-- NUR der v_weight-Block ändert sich: weight = MAX(Abo-Gewicht, Fan-Rang-Gewicht).
--   Abo (Bestand 336): aktives Club-Abo → 2×.
--   Fan-Rang (neu): ultra/legende → 2×, ehrenmitglied/vereinsikone → 3×, sonst 1×.
--   MAX = "integriert, nicht doppelt" + Abo-Floor (keine Regression der Live-2×). Tally-only (D86).
-- Geld-Branches (Wallet-Abzug, Treasury, transactions, amount_paid/creator_share/platform_share) BYTE-IDENTISCH (D87/Slice 156).
-- Source-of-truth: 20260618170000_slice_336_polls_social.sql
