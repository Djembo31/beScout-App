-- Slice 385 — E-3 Aufstellungs-Regel-Fundament (Topf 2, Weg B, D107)
-- ============================================================================
-- 1) Additive JSONB-Spalte events.lineup_rules: Liste typisierter Aufstellungs-
--    Bedingungen, z.B. [{"type":"min_per_own_club","value":5}]. NULL/[] = keine Regel.
-- 2) rpc_save_lineup CREATE OR REPLACE gegen LIVE-Baseline (pg_get_functiondef,
--    D87/PATCH-AUDIT): EINZIGE Änderungen sind additiv:
--      - 4 neue DECLARE-Variablen (v_rule, v_rule_type, v_rule_value, v_own_club_count)
--      - EIN generischer Validator-Block nach der E-1-Liga-Bindung, VOR INSERT/UPDATE
--        + VOR Wildcard-spend/earn → bei Reject wird KEINE Ressource bewegt.
--      - Erste Pilot-Regel: min_per_own_club (E-3a, FESTE ZAHL, CEO-approved 2026-06-25).
--    Alle bestehenden Blöcke (max_per_club, E-1 Liga-Bindung, salary_cap, bench,
--    holding_locks, Track-F Wildcard) bleiben byte-identisch.
-- ============================================================================

ALTER TABLE public.events ADD COLUMN IF NOT EXISTS lineup_rules jsonb;

COMMENT ON COLUMN public.events.lineup_rules IS
  'Slice 385 (E-3, D107): Liste typisierter Aufstellungs-Regeln (Weg B). '
  '[{"type":"min_per_own_club","value":5}]. NULL/[] = keine Regel. '
  'Generischer Validator in rpc_save_lineup, fail-closed bei unbekanntem type.';
...
DECLARE
  ...
  v_bench_uids UUID[];
  v_event_league_id UUID;  -- Track F addition
  v_rule JSONB; v_rule_type TEXT; v_rule_value INT; v_own_club_count INT;  -- E-3 (Slice 385) addition
BEGIN
  ...
  -- E-1 (Slice 380): Liga-Bindung. Ist das Event an eine Fußball-Liga gebunden,
  -- müssen ALLE aufgestellten Spieler (Starter + Bank) zu einem Verein dieser Liga
  -- gehören. JOIN auf clubs deckt club_id IS NULL fail-closed ab (kein Row -> reject).
  ...
  -- E-3 (Slice 385, D107): generischer Aufstellungs-Regel-Validator (Weg B).
  -- Liest events.lineup_rules (JSONB-Liste typisierter Bedingungen) ab. Fail-closed
  -- bei unbekanntem Typ. Läuft VOR INSERT/UPDATE + VOR jeder Wildcard-Bewegung →
  -- bei Reject wird KEINE Ressource bewegt. Erste Pilot-Regel: min_per_own_club (E-3a).
  IF v_event.lineup_rules IS NOT NULL AND jsonb_typeof(v_event.lineup_rules) = 'array'
    ...
    FOR v_rule IN SELECT * FROM jsonb_array_elements(v_event.lineup_rules) LOOP
      ...
      IF v_rule_type IS NULL OR v_rule_type NOT IN ('min_per_own_club') THEN
        RETURN jsonb_build_object('ok', false, 'error', 'unknown_lineup_rule', 'rule', v_rule_type);
      END IF;

      -- Wert-Bounds (int 1..11), defensiv VOR Cast: nicht-numerisch -> sauberer Reject
      -- statt PL/pgSQL-Cast-Exception (Pre-Mortem #5).
      ...
      IF v_rule_type = 'min_per_own_club' THEN
        -- Zählt Starter (v_all_slots, 12 Slots — konsistent mit max_per_club, das
        -- ebenfalls nur Starter zählt) aus dem Event-Verein. club_id NULL -> kein
        -- Treffer -> 0 -> fail-closed.
  ...
  -- Track F: lookup event.club.league_id for per-league wildcard tracking
  ...
  IF v_wildcard_delta != 0 THEN
    ...
    ELSE
      PERFORM public.earn_wildcards(
        p_user_id,
        ABS(v_wildcard_delta),
        'lineup_wildcard_refund',
        v_event_league_id,
        p_event_id,
...
-- AR-44: ACL bei CREATE OR REPLACE einer bestehenden Funktion bleibt erhalten;
-- Block dennoch idempotent gesetzt (gleiche Grants wie zuvor: authenticated + service_role,
-- Cron/Trigger ruft via service_role). 22-arg-Signatur exakt.
