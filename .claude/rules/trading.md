...
- **Display vs Charge bei eigener Order (S7-303 F-1) — Eigene-Order-Exclusion ist Welle-1.6-Standard (S414/S415/S416, KOMPLETT):** Jede Kauf-Anzeige (Best-Ask/Spread/Depth/„sofort kaufbar"-Liste) MUSS eigene Sell-Orders excludieren (`sellOrders.filter(o => !o.is_own)`), und jede Markt-Nachfrage-Anzeige (Best-Bid/„Höchstes Gesuch") die eigenen Kaufgebote (`bids.filter(b => b.sender_id !== userId)`), weil `buy_player_sc`/`buy_from_order` nur Fremd-Orders matchen (`user_id != p_user_id`) bzw. `accept_offer` das eigene Gebot blockt (sender ≠ acceptor). `PublicOrder.is_own` ist server-projiziert (`get_public_orderbook`); `OfferWithDetails` hat KEIN is_own, aber `sender_id` (erbt `DbOffer`) → Bid-Exclusion läuft client-seitig per `sender_id`, **kein Type/Service-Change** (Handoff-Annahme war falsch). **SSOT der Bid-Regel = `src/lib/orderbook.ts` (`excludeOwnBids`/`bestForeignBidCents`, cents-Rückgabe, S416)** gegen erneute „von-allem-N"-Drift. **Alle Surfaces gefixt:** OrderDepthView Markt-Tab (414) · OrderbookSummary/OrderbookDepth Player-Detail (415, live) · QuickStats bestBid + „sofort kaufbar"-Liste + OrderbookSummary bid + SellModal Höchstes-Gesuch/Accept-Liste (416). Ask-Seite bewusst inline `!is_own` (boolean, kein Rechen-Drift) in 5 Surfaces. OrderDepthView bid-Seite nutzt `orders`/CLOB (nicht offers, D112 Fork B). „PlayerHero bestBid" aus altem Handoff existiert nicht (= QuickStats). Bei jeder neuen Bid/Ask-Anzeige: Helper nutzen + Live-Render (errors-frontend S414/415/416).
- **Eigen-Ausschluss gilt AUCH server-seitig in „Gebote-die-ich-annehmen-kann"-Listen (S417):** der Portfolio-Tab „Offene Gebote" listet öffentliche Kaufgebote auf besessene Spieler — eigene Gebote (`sender_id == ich`) MÜSSEN da raus, sonst tote Zeile (nicht annehmbar: `accept_offer` blockt Selbst-Annahme; nicht stornierbar: Cancel lebt im „Ausgehend"-Tab `getOutgoingOffers`). Zwei autoritative Quellen, BEIDE filtern server-seitig: `getOpenBids({ownedByUserId})` → `.neq('sender_id', ownedByUserId)` (NUR ownedByUserId-Zweig; der `playerId`-Pfad/Player-Detail bleibt unberührt = Welle-1.6-Client-SSOT macht's dort) + RPC `get_market_user_dashboard` open_bids-Subquery `AND sender_id <> p_user_id`. Server-SSOT statt Consumer-Band-Aid (`BestandView.tsx:115` filterte schon client-seitig = jetzt redundant, bleibt Defense-in-Depth). Read-only, kein Geldfluss → PATCH-AUDIT (auth-guard+ACL erhalten) statt Zero-Sum. **Handoff-Korrektur:** Befund „cancel_offer_rpc nicht verkabelt" war FALSCH — Storno IST im „Ausgehend"-Tab verkabelt (`OffersTab.tsx:464`); der echte Bug war der Eigen-Gebot-Leak in den falschen Tab.
...
- Details: `docs/knowledge/domain/treasury.md` (Kanon) + `memory/decisions.md` D83 (Pfad `memory/decision_pricing_asset_model.md` existiert NICHT — Drift)
