---
# ON-DEMAND DETAIL-REFERENZ — bewusst NICHT auto-geladen (Slice 352).
# Der Navigator in errors-frontend.md (always-loaded bei .tsx-Edit) zeigt die Regel inline;
# dieses File traegt den vollen Detail (Root-Cause, Code-Bloecke, Audits) und wird NUR
# on-demand via Read geoeffnet, wenn ein Navigator-Pattern wirklich zutrifft.
# Non-matching glob (beide Keys), damit kein Edit es je auto-laedt:
paths:
  - "__never-autoload__/**"
globs:
  - "__never-autoload__/**"
...
# Errors: Frontend — Detail (on-demand)

Stand: 2026-04-24 · Split aus `common-errors.md` (Slice 186) · Slice 352: Detail-Auslagerung aus dem Navigator (`errors-frontend.md`). Siehe auch `ui-components.md`, `testing.md`.

> **Einstieg:** Du landest hier, weil ein Navigator-Eintrag in `errors-frontend.md` auf einen dieser Abschnitte zeigt. Springe zum benannten Abschnitt (« Heading »). Die Reihenfolge der Abschnitte ist identisch mit dem Navigator.
