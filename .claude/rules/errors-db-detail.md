---
# ON-DEMAND DETAIL-REFERENZ — bewusst NICHT auto-geladen (Slice 353).
# Navigator in errors-db.md zeigt die Regel inline; dieses File trägt vollen Detail
# (Root-Cause, Code, Audits) und wird NUR on-demand via Read geöffnet.
paths:
  - "__never-autoload__/**"
globs:
  - "__never-autoload__/**"
...
# Errors: DB — Detail (on-demand)

Stand: 2026-05-05 · Split aus `common-errors.md` (Slice 186) · Slice 353: Detail-Auslagerung aus dem Navigator (`errors-db.md`). Siehe auch `database.md` (Columns, CHECK), `trading.md` (Money-Regeln).

> **Einstieg:** Du landest hier, weil ein Navigator-Eintrag in `errors-db.md` auf einen dieser Abschnitte zeigt. Springe zum benannten Abschnitt (« Heading »). Die Reihenfolge der Abschnitte ist identisch mit dem Navigator.
...
**Verallgemeinerung (Status-CHECK-Drift, Slice 332):** Gleiche Klasse auf JEDER `status`/`type`-CHECK-Achse. `bounties_status_check` kannte `'open'/'closed'/'cancelled'`, aber `approve_bounty_submission` setzt `status='completed'` → JEDE Bounty-Approval failte mit 23514 (latent: 0 approved submissions je). **Regel:** Bei jeder RPC, die ein Status-/Type-Literal in eine Spalte mit CHECK schreibt → `pg_get_constraintdef('<table>_<col>_check')` gegen ALLE in RPC-Bodies geschriebenen Literale matchen (`grep -oE "status\s*=\s*'[a-z_]+'" supabase/migrations/`). Drei Fälle EINE Session (transactions.type/liquidation · events.status/'cancelled'-UI · bounties.status/'completed') zeigen: die Falle ist systemisch. **Reference:** Slice 332 `20260617160000_slice_332_club_bounties_treasury.sql` (status-CHECK +'completed').

**Vierter Fall — `ticket_transactions.source` (Slice 335, 2026-06-18):** `rpc_cancel_event_entries` schrieb beim Ticket-Refund `source='event_entry_refund'`, aber `ticket_transactions_source_check` kannte den Wert nie → JEDE Ticket-Entry-Erstattung wäre mit 23514 gescheitert. **Doppelt latent:** (a) RPC nirgends verdrahtet (0 Consumer) UND (b) selbst der platform-admin-Wrapper hätte beim ersten echten Aufruf gefailt. Gefangen erst durch den **Live-Money-Smoke** (`BEGIN;…ROLLBACK;`) beim Verdrahten — NICHT durch tsc/Unit-Tests/grep. **Lehre:** Beim Verdrahten einer bisher toten RPC, die in eine Spalte mit CHECK schreibt, IMMER einen Live-Money-Smoke fahren — eine „existierende, getestet aussehende" RPC kann eine nie ausgeführte CHECK-Falle tragen. `transactions_type_check` kannte den Schwester-Wert `event_entry_unlock` ($SCOUT-Pfad) bereits → nur die Ticket-Achse war offen. **Reference:** Slice 335 `20260618160000_slice_335_event_cancel.sql` §1b.

**Umgekehrte Richtung — RPC-Guard an bestehenden CHECK angleichen (Slice 340, 2026-06-18):** Gleiche Drift-Familie, andere Heilrichtung. Hier war der **CHECK** die Wahrheit und der **RPC-Guard** gedriftet: `bounties_reward_cents_check` = `reward_cents BETWEEN 500 AND 100000`, aber `create_user_bounty` guardete nur `< 100` (min, zu niedrig) und hatte **keinen Max-Guard** (ein früheres CREATE OR REPLACE hatte den alten 100M-Guard entfernt). → reward 100–499 / >100000 cents passierte den RPC-Guard und schlug am CHECK mit rohem 23514 statt sauberem `{success:false,error}` fehl (Geld via CHECK geschützt, aber UX-Crash). **Regel:** Schreibt eine RPC in eine Spalte mit Wertebereichs-CHECK, MUSS der RPC-Guard den CHECK spiegeln — mit **strikten** Vergleichen `< min` / `> max` (nicht `<=`/`>=`), damit die inklusiven CHECK-Grenzen erlaubt bleiben (off-by-one-Falle). Detection: `pg_get_constraintdef('<table>_<col>_check')` gegen die Guard-Literale matchen — via **live `pg_get_functiondef`** (D87), nicht die alte Migration, da der Guard durch späteres CREATE OR REPLACE gedriftet sein kann. **Reference:** Slice 340 `20260618210000_slice_340_bounty_reward_guard.sql`.
